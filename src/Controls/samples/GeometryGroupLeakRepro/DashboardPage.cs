...
public sealed class DashboardPage : ContentPage
{
	...
	async Task RunAsync(ReproMode mode)
	{
		...
		try
		{
			...
			for (var i = 0; i < options.Cycles; i++)
			{
				...
				await Shell.Current.GoToAsync(AppShell.GeometryLeakRoute, animate: false);
	...
	async Task RunAllScenariosAsync()
	{
		...
		{
			$"GeometryGroupLeakRepro autorun started at {DateTimeOffset.Now:O}",
			$"Defaults: pages={_cyclesEntry.Text}, payloadMB={_payloadEntry.Text}, cards={_cardsEntry.Text}, fragmentsPerCard={_fragmentsEntry.Text}, dwellMs={_dwellEntry.Text}"
		};

		foreach (var mode in new[]
		{
			ReproMode.FreshFragmentsControl,
			ReproMode.SharedFragments,
			ReproMode.RemoveSharedFragmentsIndividually
		})
		{
			await RunAsync(mode);
	...
	static async Task WriteAutoRunReportAsync(string reportText)
	{
		...
		paths.Add(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "GeometryGroupLeakRepro", "autorun-results.txt"));
		paths.Add(Path.Combine(Path.GetTempPath(), "geometrygroupleakrepro-results.txt"));
