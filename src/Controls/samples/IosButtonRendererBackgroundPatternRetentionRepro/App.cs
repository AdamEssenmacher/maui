...
sealed class RunnerPage : ContentPage
{
	...
	public RunnerPage()
	{
		Content = new Label
		{
			Text = "Running iOS ButtonRenderer background pattern retention repro...",
	...
	async Task TryRunAsync()
	{
		...
		{
			var text = "IosButtonRendererBackgroundPatternRetentionRepro ERROR" + Environment.NewLine + ex;
			File.WriteAllText(ReproSession.ResultsPath, text);
			Console.WriteLine(text);
		}
