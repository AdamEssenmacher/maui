...
sealed class RunnerPage : ContentPage
{
	...
	public RunnerPage()
	{
		Content = new Label
		{
			Text = "Running iOS ShellSection tab bar accessibility retention repro...",
	...
	async Task TryRunAsync()
	{
		...
		{
			var text = "IosShellSectionTabBarItemAccessibilityRetentionRepro ERROR" + Environment.NewLine + ex;
			File.WriteAllText(ReproSession.ResultsPath, text);
			Console.WriteLine(text);
			await Task.Delay(250);
			Environment.Exit(3);
