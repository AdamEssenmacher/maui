...
internal static class AutoRunSettings
{
...
#else
		return ResultsPath ?? Path.Combine(Path.GetTempPath(), "webauthenticatorcompletedoptionsretentionleakrepro-results.txt");
#endif
	}
	...
	public static void Initialize(string[] args)
	{
		...
		if (string.Equals(Environment.GetEnvironmentVariable("WEB_AUTHENTICATOR_COMPLETED_OPTIONS_RETENTION_LEAK_REPRO_AUTORUN"), "1", StringComparison.Ordinal))
			Enabled = true;

		ResultsPath ??= Environment.GetEnvironmentVariable("WEB_AUTHENTICATOR_COMPLETED_OPTIONS_RETENTION_LEAK_REPRO_RESULTS");
	}
}
