...
sealed class RunnerPage : ContentPage
{
	...
	public RunnerPage()
	{
		Content = new Label
		{
			Text = "Running iOS Shell SearchHandler accessibility identifier retention repro...",
	...
	async Task TryRunAsync()
	{
		...
		{
			var text = "IosShellSearchHandlerAccessibilityIdRetentionRepro ERROR" + Environment.NewLine + ex;
			File.WriteAllText(ReproSession.ResultsPath, text);
			Console.WriteLine(text);
		}
