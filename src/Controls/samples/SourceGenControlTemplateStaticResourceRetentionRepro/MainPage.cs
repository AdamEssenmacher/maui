...
public sealed class MainPage : ContentPage
{
	...
	public MainPage(string? resultsPath)
	{
		...
		Title = "SourceGen ControlTemplate StaticResource Retention";

		_status = new Label
		{
			Text = "Running SourceGen ControlTemplate StaticResource retention repro...",
	...
	async void OnLoaded(object? sender, EventArgs e)
	{
		...
		try
		{
			...
			_status.Text = result.Proven
				? "PROVEN: escaped SourceGen ControlTemplates retained discarded page roots."
		...
		{
			var report = "SourceGen ControlTemplate StaticResource retention repro failed." + Environment.NewLine + ex;
			_status.Text = report;
	...
	{
		var pageRefs = new List<WeakReference<SourceGenControlTemplatePage>>(PageCount);
	...
	static void CreateAndDropPage(
		...
		List<WeakReference<SourceGenControlTemplatePage>> pageRefs,
		...
		List<WeakReference<byte[]>> payloadBufferRefs)
	{
		...
		var page = new SourceGenControlTemplatePage
		{
			Title = $"SourceGen control-template page {index}",
			BindingContext = payload
		};

		if (page.Resources["EscapedTemplate"] is not ControlTemplate template)
			throw new InvalidOperationException("The SourceGen page did not contain the expected ControlTemplate resource.");

		var createdContent = template.CreateContent();
		if (createdContent is not Grid grid || grid.Children.FirstOrDefault() is not Label label || label.Text != "SourceGen ControlTemplate StaticResource content")
			throw new InvalidOperationException("The SourceGen ControlTemplate did not resolve the page-local StaticResource.");

		templates.Add(template);
		pageRefs.Add(new WeakReference<SourceGenControlTemplatePage>(page));
	...
	readonly record struct ReproResult(ScenarioResult Control, ScenarioResult Current, bool Proven)
	{
		public string ToReport()
		{
			...
			builder.AppendLine("SourceGen ControlTemplate StaticResource root-retention repro");
			...
			builder.AppendLine("Escaped ControlTemplate -> SourceGen LoadTemplate delegate -> generated XamlServiceProvider(this) capture -> SourceGenControlTemplatePage -> BindingContext payload");
