...
internal static class AutoRunSettings
{
	...
	{
		return ResultsPath ?? Path.Combine(Path.GetTempPath(), "ios-actionsheet-observer-retention-results.txt");
	}
	...
	public static void Initialize(string[] args)
	{
		...
		if (string.Equals(Environment.GetEnvironmentVariable("IOS_ACTIONSHEET_OBSERVER_RETENTION_REPRO_AUTORUN"), "1", StringComparison.Ordinal))
			Enabled = true;

		ResultsPath ??= Environment.GetEnvironmentVariable("IOS_ACTIONSHEET_OBSERVER_RETENTION_REPRO_RESULTS");
	}
}
