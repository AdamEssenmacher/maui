...
sealed class RunnerPage : ContentPage
{
	...
	public RunnerPage()
	{
		Content = new Label
		{
			Text = "Running Mac Catalyst TabbedPage handler ItemsSource reset owner retention repro...",
	...
	async Task TryRunAsync()
	{
		...
		{
			var text = "MacCatalystTabbedPageHandlerItemsSourceResetOwnerRetentionRepro ERROR" + Environment.NewLine + ex;
			File.WriteAllText(ReproSession.ResultsPath, text);
			Console.WriteLine(text);
		}
