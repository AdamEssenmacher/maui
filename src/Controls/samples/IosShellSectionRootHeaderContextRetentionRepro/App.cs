...
namespace IosShellSectionRootHeaderContextRetentionRepro;
...
sealed class RunnerPage : ContentPage
{
	...
	public RunnerPage()
	{
		Content = new Label
		{
			Text = "Running iOS ShellSectionRootHeader context retention repro...",
	...
	async Task TryRunAsync()
	{
		...
		{
			var text = "IosShellSectionRootHeaderContextRetentionRepro ERROR" + Environment.NewLine + ex;
			File.WriteAllText(ReproSession.ResultsPath, text);
			Console.WriteLine(text);
...
{
	public const string ResultsPath = "/tmp/ios-shellsectionrootheader-context-retention-results.txt";

	const int Attempts = 96;
	const int PayloadBytes = 1024 * 1024;
	const int ShellContentsPerSection = 4;

	static readonly FieldInfo ShellContextField =
		typeof(ShellSectionRootHeader).GetField("_shellContext", BindingFlags.Instance | BindingFlags.NonPublic)
		?? throw new MissingMemberException(typeof(ShellSectionRootHeader).FullName, "_shellContext");

	static readonly List<IReadOnlyList<ShellSectionRootHeader>> RetainedHeaderSets = new();
	...
	public static async Task<ReproReport> RunAsync(IMauiContext hostContext)
	{
		...
		var control = await RunScenarioAsync(
			hostContext,
			"control: ShellSectionRootHeader.Dispose() plus private _shellContext clear",
			clearContextField: true);

		var current = await RunScenarioAsync(
			hostContext,
			"current: ShellSectionRootHeader.Dispose() only",
			clearContextField: false);

		ForceFullGc();
		var final = GC.GetTotalMemory(forceFullCollection: true);

		GC.KeepAlive(RetainedHeaderSets);
		return new ReproReport(Attempts, PayloadBytes, ShellContentsPerSection, baseline, final, control, current);
	}

	static async Task<RunStats> RunScenarioAsync(IMauiContext hostContext, string name, bool clearContextField)
	{
		var retainedHeaders = new List<ShellSectionRootHeader>(Attempts);
		var headerRefs = new List<WeakReference<ShellSectionRootHeader>>(Attempts);
		...
		RetainedHeaderSets.Add(retainedHeaders);
		await DrainMainQueueAsync();
		ForceFullGc();
	...
	static void CreateDisposedHeader(
		IMauiContext hostContext,
		int index,
		bool clearContextField,
		List<ShellSectionRootHeader> retainedHeaders,
		List<WeakReference<ShellSectionRootHeader>> headerRefs,
		...
		List<PayloadWeakReferences> payloadRefs)
	{
		...
		var shell = CreateShell(index, payload, out var shellItem, out var shellSection);
		...
		var header = new ShellSectionRootHeader(shellContext)
		{
		...
		headerRefs.Add(new WeakReference<ShellSectionRootHeader>(header));
		...
		shellSectionRefs.Add(new WeakReference<ShellSection>(shellSection));
		...
		if (clearContextField)
			ShellContextField.SetValue(header, null);

		retainedHeaders.Add(header);
	}

	static Shell CreateShell(int index, PayloadService payload, out ShellItem shellItem, out ShellSection shellSection)
	{
		var shell = new Shell
		{
		...
		{
			Title = $"North region {index:000}",
			BindingContext = payload
		};

		var section = new ShellSection
		{
			Title = $"Case workspaces {index:000}",
			BindingContext = payload
		};

		for (var contentIndex = 0; contentIndex < ShellContentsPerSection; contentIndex++)
		{
			var content = new ShellContent
			{
				Title = $"Queue {contentIndex + 1}",
				Content = new ContentPage
				{
					Title = $"Case queue {index:000}-{contentIndex + 1}",
					BindingContext = payload,
					Content = new Label { Text = $"Case payload {index:000}-{contentIndex + 1}" }
				}
			};
	...
	internal readonly record struct RunStats(
		...
		int HeadersWithContext,
		int HeadersResolvingPayloadService,
		long RetainedPayloadBytes)
	{
		internal static RunStats From(
			string name,
			IReadOnlyList<WeakReference<ShellSectionRootHeader>> headerRefs,
		...
		{
			var headersWithContext = 0;
			var headersResolvingPayloadService = 0;

			foreach (var headerRef in headerRefs)
			{
				if (!headerRef.TryGetTarget(out var header))
					continue;

				if (ShellContextField.GetValue(header) is PayloadShellContext context)
				{
					headersWithContext++;

					if (context.Shell.Handler?.MauiContext?.Services.GetService(typeof(PayloadService)) is PayloadService)
						headersResolvingPayloadService++;
				}
			}
	...
	ReproSession.RunStats Current)
{
	public bool LeakProved =>
		Control.AliveHeaders == Attempts &&
		Control.HeadersWithContext == 0 &&
		Control.HeadersResolvingPayloadService == 0 &&
		...
		Current.AliveHeaders == Attempts &&
		Current.HeadersWithContext == Attempts &&
		Current.HeadersResolvingPayloadService == Attempts &&
	...
	public string ToText()
	{
		...
		builder.AppendLine("IosShellSectionRootHeaderContextRetentionRepro");
		builder.AppendLine($"RESULT: {(LeakProved ? "PROVEN" : "NOT PROVEN")}");
		builder.AppendLine($"ShellSectionRootHeader peers retained per scenario: {Attempts}");
		builder.AppendLine($"ShellContent items per section: {ShellContentsPerSection}");
		...
		builder.AppendLine("Leak path: retained disposed ShellSectionRootHeader native peer -> readonly _shellContext -> Shell -> Handler.MauiContext -> service provider -> payload service/byte array.");
		builder.AppendLine("Distinct from ShellSectionRootHeader native label text retention: this proof clears no native label slots and measures Shell/service context retention through the disposed header object itself.");
	...
	static void AppendScenario(StringBuilder builder, ReproSession.RunStats stats)
	{
		...
		builder.AppendLine($"  disposed ShellSectionRootHeaders alive after full GC: {stats.AliveHeaders}/{stats.Attempts}");
		builder.AppendLine($"  header _shellContext fields alive: {stats.HeadersWithContext}/{stats.Attempts}");
		builder.AppendLine($"  headers resolving payload service through Shell.Handler.MauiContext: {stats.HeadersResolvingPayloadService}/{stats.Attempts}");
		...
		builder.AppendLine($"  ShellSections alive after full GC: {stats.AliveShellSections}/{stats.Attempts}");
