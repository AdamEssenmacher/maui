...
namespace AndroidEmptyViewAdapterHeaderFooterHolderRetentionRepro;
...
internal static class ReproSession
{
	...
	public static async Task<ReproReport> RunAsync(IMauiContext rootContext)
	{
		...
		var current = await RunScenarioAsync(
			rootContext.Services,
			androidContext,
			"current: EmptyViewAdapter SimpleViewHolder recycle skips plain ItemContentView cleanup",
		...
		bool explicitHolderCleanup)
	{
		...
		var collectionView = new CollectionView
		{
			Header = header,
			Footer = footer,
			EmptyView = "No orders match the current filter.",
		...
		var adapter = new EmptyViewAdapter(collectionView)
		{
			Header = header,
			Footer = footer,
			EmptyView = collectionView.EmptyView
		};
	...
	ReproSession.ScenarioResult Current)
{
	...
	public string ToText()
	{
		...
		return string.Join(Environment.NewLine,
			"AndroidEmptyViewAdapterHeaderFooterHolderRetentionRepro",
			$"Cycles: {Cycles}",
