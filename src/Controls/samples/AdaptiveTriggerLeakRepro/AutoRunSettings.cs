...
internal static class AutoRunSettings
{
	...
	public static void Initialize(string[] args)
	{
		...
		if (string.Equals(Environment.GetEnvironmentVariable("ADAPTIVE_TRIGGER_LEAK_REPRO_AUTORUN"), "1", StringComparison.Ordinal))
			Enabled = true;

		ResultsPath ??= Environment.GetEnvironmentVariable("ADAPTIVE_TRIGGER_LEAK_REPRO_RESULTS");
	}
}
