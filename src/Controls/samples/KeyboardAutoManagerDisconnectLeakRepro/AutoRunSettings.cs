...
internal static class AutoRunSettings
{
	...
	{
		return ResultsPath ?? Path.Combine(Path.GetTempPath(), "keyboardautomanagerdisconnectleakrepro-results.txt");
	}
	...
	public static void Initialize(string[] args)
	{
		...
		if (string.Equals(Environment.GetEnvironmentVariable("KEYBOARD_AUTO_MANAGER_DISCONNECT_LEAK_REPRO_AUTORUN"), "1", StringComparison.Ordinal))
			Enabled = true;

		ResultsPath ??= Environment.GetEnvironmentVariable("KEYBOARD_AUTO_MANAGER_DISCONNECT_LEAK_REPRO_RESULTS");
	}
}
