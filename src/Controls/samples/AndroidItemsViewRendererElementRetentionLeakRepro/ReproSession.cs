	...
	long ManagedHeapFinal)
{
	...
	string Format(RunStats stats)
	{
		return string.Join(Environment.NewLine,
			...
			$"  CollectionViews alive after full GC: {stats.AliveElements}/{stats.Attempts}",
...
internal static class ReproSession
{
	...
	static readonly FieldInfo ItemsViewField = FindField(typeof(CollectionViewRenderer), "ItemsView");
	static readonly FieldInfo StructuredItemsViewField = FindField(typeof(CollectionViewRenderer), "_itemsView");
	static readonly FieldInfo ItemsViewAdapterField = FindField(typeof(CollectionViewRenderer), "ItemsViewAdapter");
	static readonly FieldInfo EmptyViewAdapterField = FindField(typeof(CollectionViewRenderer), "_emptyViewAdapter");
	...
	public static async Task<ReproReport> RunAsync(IMauiContext mauiContext)
	{
		...
		var control = await RunScenarioAsync(
			mauiContext,
			"control: dispose then clear stale ItemsViewRenderer fields",
			clearRendererElement: true);

		var current = await RunScenarioAsync(
			mauiContext,
			"current: disposed ItemsViewRenderer keeps ItemsView fields",
	...
	static async Task<RunStats> RunScenarioAsync(IMauiContext mauiContext, string name, bool clearRendererElement)
	{
		...
		var rendererRefs = new List<WeakReference<CollectionViewRenderer>>(Attempts);
		var elementRefs = new List<WeakReference<CollectionView>>(Attempts);
	...
	static void CreateDisposedRenderer(
		...
		List<WeakReference<CollectionViewRenderer>> rendererRefs,
		List<WeakReference<CollectionView>> elementRefs,
		List<PayloadWeakReference> payloadRefs,
		int index)
	{
		var payload = new Payload(index, PayloadBytes);
		var element = new CollectionView
		{
			ItemsLayout = LinearItemsLayout.Vertical,
			ItemsSource = Enumerable.Range(0, 12).Select(item => $"Order {index}-{item}").ToArray(),
			Header = $"Batch {index}",
			BindingContext = payload
		};

		var renderer = new CollectionViewRenderer(mauiContext.Context ?? throw new InvalidOperationException("Android context is not available."));
		...
		elementRefs.Add(new WeakReference<CollectionView>(element));
		rendererRefs.Add(new WeakReference<CollectionViewRenderer>(renderer));
		...
		if (clearRendererElement)
			ClearRendererReferences(renderer);
	}

	static void ClearRendererReferences(CollectionViewRenderer renderer)
	{
		ItemsViewField.SetValue(renderer, null);
		StructuredItemsViewField.SetValue(renderer, null);
		ItemsViewAdapterField.SetValue(renderer, null);
		EmptyViewAdapterField.SetValue(renderer, null);
	}
