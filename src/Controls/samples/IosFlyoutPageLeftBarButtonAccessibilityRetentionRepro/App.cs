...
sealed class RunnerPage : ContentPage
{
	...
	public RunnerPage()
	{
		Content = new Label
		{
			Text = "Running iOS FlyoutPage left bar button accessibility retention repro...",
	...
	async Task TryRunAsync()
	{
		...
		{
			var text = "IosFlyoutPageLeftBarButtonAccessibilityRetentionRepro ERROR" + Environment.NewLine + ex;
			File.WriteAllText(ReproSession.ResultsPath, text);
			Console.WriteLine(text);
		}
