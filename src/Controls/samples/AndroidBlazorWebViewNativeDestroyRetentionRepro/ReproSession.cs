...
{
	const int Cycles = 2;
	const int HtmlPayloadChars = 1024 * 1024;
	...
	public static async Task<ReproReport> RunAsync(IMauiContext context)
	{
		...
		var control = await RunScenarioAsync(
			"control: disconnect BlazorWebViewHandler, then explicitly clear/destroy retained native WebView peers",
			context,
			explicitNativeDestroy: true);

		var current = await RunScenarioAsync(
			"current: BlazorWebViewHandler.DisconnectHandler() leaves retained native WebView peers not destroyed",
			context,
		...
		bool explicitNativeDestroy)
	{
		...
		var handler = new TrackingBlazorWebViewHandler();
		handler.SetMauiContext(context);
		handler.SetVirtualView(blazorWebView);

		var nativeWebView = handler.TrackingWebView
			?? throw new InvalidOperationException("TrackingBlazorWebViewHandler did not create a native WebView.");
		...
		blazorWebView.BindingContext = null;
	...
	{
		var prefix = $"<html><body><h1>Blazor work order {cycle:D4}</h1><script>window.payload='";
	...
	internal sealed class TrackingBlazorWebViewHandler : BlazorWebViewHandler
	{
		public TrackingBlazorAndroidWebView? TrackingWebView { get; private set; }

		protected override AWebView CreatePlatformView()
		{
			var context = Context ?? throw new InvalidOperationException("Android context is not available.");
			var platformView = new TrackingBlazorAndroidWebView(context)
			{
			...
			// Keep this repro focused on native WebView destruction, not the already tracked C043
			// Blazor WebViewClient/manager strong-handler root.
	...
	internal sealed class TrackingBlazorAndroidWebView : AWebView
	{
		public TrackingBlazorAndroidWebView(Context context)
			: base(context)
		{
		}
	...
	internal sealed record NativePeerRoot(IntPtr GlobalRef, int RequestedHtmlChars)
	{
		...
		public static NativePeerRoot Create(TrackingBlazorAndroidWebView webView, int requestedHtmlChars)
		{
	...
	internal sealed record TrackedCycle(
		...
		WeakReference<TrackingBlazorWebViewHandler> Handler,
		WeakReference<BlazorWebView> VirtualView,
		...
		int FrameworkDestroyCalls)
	{
		public static TrackedCycle Create(
			int cycle,
			NativePeerRoot nativeWebView,
			TrackingBlazorWebViewHandler handler,
			...
			int frameworkDestroyCalls)
		{
			return new TrackedCycle(
				cycle,
				nativeWebView,
				new WeakReference<TrackingBlazorWebViewHandler>(handler),
				new WeakReference<BlazorWebView>(blazorWebView),
	...
	ReproSession.ScenarioResult Current)
{
	...
	public string ToText()
	{
		...
		return string.Join(Environment.NewLine,
			"AndroidBlazorWebViewNativeDestroyRetentionRepro",
			...
			"Source path exercised: Android BlazorWebViewHandler.DisconnectHandler()",
			"Control cleanup clears the native clients, stops loading, removes child views, and invokes native WebView.destroy() through the retained JNI peer.",
			"Current cleanup is the framework BlazorWebViewHandler.DisconnectHandler() path only.",
			"Native clients are intentionally neutral to isolate this from C043 managed BlazorWebView handler retention.",
	...
	static string Format(ReproSession.ScenarioResult result)
	{
		return string.Join(Environment.NewLine,
			...
			$"  alive MAUI BlazorWebViews after full GC: {result.AliveVirtualViews}/{result.TrackedCycles}",
