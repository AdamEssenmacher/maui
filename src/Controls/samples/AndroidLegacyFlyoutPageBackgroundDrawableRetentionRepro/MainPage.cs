...
public sealed class MainPage : ContentPage
{
	...
	public MainPage()
	{
		_status = new Label
		{
			Text = "Running Android legacy flyout page background drawable retention repro...",
			Margin = 24
		};
	...
	async void OnLoaded(object? sender, EventArgs e)
	{
		...
		try
		{
			...
			var reproContext = new MauiContext(context.Services, androidContext);
			Microsoft.Maui.Controls.Compatibility.Forms.Init(reproContext);
			var result = await ReproSession.RunAsync(reproContext);
			text = result.ToText();
		}
		catch (Exception ex)
		{
