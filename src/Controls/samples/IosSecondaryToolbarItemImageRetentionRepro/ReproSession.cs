...
internal static class ReproSession
{
	...
	public static readonly string ResultsPath =
		Path.Combine("/tmp", "ios-secondary-toolbaritem-image-retention-results.txt");

	public static async Task<ReproReport> RunAsync(IMauiContext context)
	{
		WriteProgress("Starting iOS secondary ToolbarItem image retention repro.");
		ForceFullGc();
		var baselineBytes = GC.GetTotalMemory(true);

		WriteProgress("Running control scenario.");
		var control = await RunScenarioAsync(
			"control: clear secondary toolbar custom-view image before retaining peer",
			context,
			clearNativeImageAndDisposeResult: true);

		WriteProgress("Running current MAUI scenario.");
		var current = await RunScenarioAsync(
			"current: MAUI SecondaryToolbarItem leaves custom-view image assigned",
			context,
		...
		bool clearNativeImageAndDisposeResult)
	{
		...
		var nativeItem = clearNativeImageAndDisposeResult
			? await CreateSecondaryToolbarItemWithDisposedResultAsync(toolbarItem, source, context)
			: await CreateSecondaryToolbarItemWithCurrentPathAsync(toolbarItem);

		var imageView = FindNestedImageView(nativeItem.CustomView)
			?? throw new InvalidOperationException("Secondary toolbar item custom view did not contain a UIImageView.");

		if (imageView.Image is null)
			throw new InvalidOperationException("Secondary toolbar item conversion did not assign a native UIImage.");
	...
	static async Task<UIBarButtonItem> CreateSecondaryToolbarItemWithCurrentPathAsync(ToolbarItem toolbarItem)
	{
		var nativeItem = toolbarItem.ToUIBarButtonItem(forceName: false, forcePrimary: false);
		await DrainMainQueueAsync();
		return nativeItem;
	}

	static async Task<UIBarButtonItem> CreateSecondaryToolbarItemWithDisposedResultAsync(
		ToolbarItem toolbarItem,
		TrackingImageSource source,
		IMauiContext context)
	{
		var nativeContent = new UIControl();
		var imageView = new UIImageView { BackgroundColor = UIColor.Clear };
		nativeContent.AddSubview(imageView);

		var nativeItem = new UIBarButtonItem(nativeContent)
		{
		...
		try
		{
			imageView.Image = result?.Value ?? throw new InvalidOperationException("Image source service returned no image.");
			await DrainMainQueueAsync();
		}
		finally
		{
	...
	static UIImageView? FindNestedImageView(UIView? view)
	{
		if (view is null)
			return null;
		...
		{
			if (FindNestedImageView(subview) is { } nested)
				return nested;
		}
		...
		int AliveSources)
	{
			...
			IReadOnlyList<TrackedCycle> tracked)
		{
			...
			{
				if (FindNestedImageView(retainedPeer.Peer.CustomView)?.Image is UIImage image)
				{
	...
	ReproSession.ScenarioResult Current)
{
	...
	public string ToText()
	{
		...
		return string.Join(Environment.NewLine,
			"IosSecondaryToolbarItemImageRetentionRepro",
			$"Cycles: {Cycles}",
