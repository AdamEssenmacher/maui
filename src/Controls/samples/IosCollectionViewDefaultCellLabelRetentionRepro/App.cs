...
sealed class RunnerPage : ContentPage
{
	...
	public RunnerPage()
	{
		Content = new Label
		{
			Text = "Running iOS CollectionView default-cell label retention repro...",
	...
	async Task TryRunAsync()
	{
		...
		{
			var text = "IosCollectionViewDefaultCellLabelRetentionRepro ERROR" + Environment.NewLine + ex;
			File.WriteAllText(ReproSession.ResultsPath, text);
			Console.WriteLine(text);
		}
