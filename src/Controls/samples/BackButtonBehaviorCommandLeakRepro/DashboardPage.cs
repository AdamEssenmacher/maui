...
public sealed class DashboardPage : ContentPage
{
	...
	{
		Title = "BackButtonBehavior Command Leak Repro";
		...
		_runLeakButton = CreateButton("Run shared command", () => RunAsync(ReproMode.SharedBackCommand));
		_runControlButton = CreateButton("Run control", () => RunAsync(ReproMode.CreatedPageControl));
		_runMitigationButton = CreateButton("Run cleanup", () => RunAsync(ReproMode.SharedBackCommandCleared));
		...
		_progress = new ProgressBar
		{
			Progress = 0,
			HeightRequest = 6,
			ProgressColor = Color.FromArgb("#475B44")
		};

		_statusLabel = new Label
		{
			Text = "Ready. Run the control first, then the shared-command scenario.",
		...
		{
			Text = "Each run creates pages with Shell BackButtonBehavior payloads, forces full GC, and counts which weak references survived.",
			TextColor = Color.FromArgb("#172026"),
		...
		Content = new ScrollView
		{
			Content = new VerticalStackLayout
			{
				...
				Children =
				{
					new Label
					{
						Text = "BackButtonBehavior.Command strong ICommand retention",
						FontSize = 22,
					...
					new Label
					{
						Text = "This models Shell pages with shared back commands used for unsaved-change guards or navigation policy.",
						FontSize = 14,
	...
	static Button CreateButton(string text, Func<Task> action)
	{
		var button = new Button
		{
			Text = text,
			FontSize = 14,
			BackgroundColor = Color.FromArgb("#475B44"),
	...
	async Task RunAsync(ReproMode mode)
	{
		...
		try
		{
			...
			for (var i = 0; i < options.Cycles; i++)
			{
				...
				BackButtonLeakPage? page = session.CreateTrackedPage();
	...
	async Task RunAllScenariosAsync()
	{
		...
		{
			$"BackButtonBehaviorCommandLeakRepro autorun started at {DateTimeOffset.Now:O}",
		...
		foreach (var mode in new[]
		{
			...
			ReproMode.SharedBackCommand,
			ReproMode.SharedBackCommandCleared
		})
		{
			await RunAsync(mode);
	...
	static async Task WriteAutoRunReportAsync(string reportText)
	{
		...
		paths.Add(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "BackButtonBehaviorCommandLeakRepro", "autorun-results.txt"));
		paths.Add(Path.Combine(Path.GetTempPath(), "backbuttonbehaviorcommandleakrepro-results.txt"));
