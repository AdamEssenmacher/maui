...
namespace IosFlyoutPageLeftBarButtonAccessibilityIdRetentionRepro;
...
internal static class ReproSession
{
	...
	public static readonly string ResultsPath =
		Path.Combine("/tmp", "ios-flyoutpage-leftbarbutton-accessibilityid-retention-results.txt");

	public static async Task<ReproReport> RunAsync(IMauiContext context)
	{
		WriteProgress("Starting iOS FlyoutPage left bar button accessibility identifier retention repro.");
		ForceFullGc();
		var baselineBytes = GC.GetTotalMemory(true);

		WriteProgress("Running control scenario.");
		var control = await RunScenarioAsync(
			"control: clear native FlyoutPage left bar button accessibility identifier",
			context,
			clearNativeIdentifier: true);

		WriteProgress("Running current MAUI scenario.");
		var current = await RunScenarioAsync(
			"current: NavigationRenderer leaves FlyoutPage left bar button accessibility identifier assigned",
			context,
		...
		bool clearNativeIdentifier)
	{
		...
		if (CountPayloadIdentifierSlots(nativeBarButtonItem) != IdentifierSlotsPerCycle)
			throw new InvalidOperationException("NavigationRenderer did not assign the expected native accessibility identifier payload.");

		// Keep this proof focused on the accessibility identifier, not adjacent image/title/action/label/hint leaks.
		...
		ClearManagedIdentifierValue(flyoutPage);
	...
	{
		var header = $"cycle-{cycle:0000}-legacy-flyout-leftbar-action-";
		var sentence = "diagnostic-route-workflow-context-command-confirmation-";
	...
	static void ClearManagedIdentifierValue(FlyoutPage flyoutPage)
	{
		flyoutPage.ClearValue(Element.AutomationIdProperty);
	}
	...
	ReproSession.ScenarioResult Current)
{
	...
	public string ToText()
	{
		...
		return string.Join(Environment.NewLine,
			"IosFlyoutPageLeftBarButtonAccessibilityIdRetentionRepro",
			$"Cycles: {Cycles}",
			...
			"Note: the known legacy left-bar action sibling keeps FlyoutPage graphs alive in both runs; this repro isolates the extra native AccessibilityIdentifier payload.",
