...
public sealed class DashboardPage : ContentPage
{
	...
	{
		Title = "WebViewSource Leak Repro";
		BackgroundColor = Colors.White;

		_cyclesEntry = CreateEntry("40");
		_payloadEntry = CreateEntry("3");
		_htmlEntry = CreateEntry("192");
		_dwellEntry = CreateEntry("100");

		_runLeakButton = CreateButton("Run shared source", () => RunAsync(ReproMode.SharedHtmlSource));
		_runControlButton = CreateButton("Run control", () => RunAsync(ReproMode.FreshHtmlSourceControl));
		_runMitigationButton = CreateButton("Run mitigation", () => RunAsync(ReproMode.ClearSharedSourceOnDisappear));
		...
		{
			Text = "Ready. Run the control first, then the shared source scenario.",
		...
		{
			Text = "Each run pushes real Shell pages containing real WebViews, unwinds the stack, forces full GC, and counts which weak references survived.",
			TextColor = Color.FromArgb("#172026"),
		...
		Content = new ScrollView
		{
			Content = new VerticalStackLayout
			{
				...
				Children =
				{
					new Label
					{
						Text = "Shared WebViewSource retention",
						FontSize = 22,
					...
					new Label
					{
						Text = "This uses realistic portal pages with cached view-model payloads. A shared HtmlWebViewSource should not keep closed pages alive.",
						FontSize = 14,
	...
	Grid CreateSettingsGrid()
	{
		...
		grid.Add(CreateField("HTML KB", _htmlEntry), 0, 1);
	...
	ReproOptions ReadOptions(ReproMode mode)
	{
		return new ReproOptions(
			...
			ReadBoundedInt(_htmlEntry.Text, 16, 2048, 192),
	...
	async Task RunAsync(ReproMode mode)
	{
		...
		try
		{
			...
			for (var i = 0; i < options.Cycles; i++)
			{
				...
				_statusLabel.Text = $"Pushing WebView page {cycle + 1}/{options.Cycles}: {options.Name}";

				await Shell.Current.GoToAsync(AppShell.WebViewLeakRoute, animate: false);
			...
			{
				token.ThrowIfCancellationRequested();
				_statusLabel.Text = $"Popping WebView page {i + 1}/{options.Cycles}: {options.Name}";
	...
	async Task RunAllScenariosAsync()
	{
		...
		{
			$"WebViewSourceLeakRepro autorun started at {DateTimeOffset.Now:O}",
			$"Defaults: pages={_cyclesEntry.Text}, payloadMB={_payloadEntry.Text}, htmlKB={_htmlEntry.Text}, dwellMs={_dwellEntry.Text}"
		};

		foreach (var mode in new[]
		{
			ReproMode.FreshHtmlSourceControl,
			ReproMode.SharedHtmlSource,
			ReproMode.ClearSharedSourceOnDisappear
		})
		{
			await RunAsync(mode);
	...
	static async Task WriteAutoRunReportAsync(string reportText)
	{
		...
		paths.Add(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "WebViewSourceLeakRepro", "autorun-results.txt"));
		paths.Add(Path.Combine(Path.GetTempPath(), "webviewsourceleakrepro-results.txt"));
