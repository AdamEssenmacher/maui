...
sealed class RunnerPage : ContentPage
{
	...
	public RunnerPage()
	{
		Content = new Label
		{
			Text = "Running Mac Catalyst compatibility Platform dialog retention repro...",
	...
	async Task TryRunAsync()
	{
		...
		try
		{
			var report = await ReproSession.RunAsync(this, context);
		...
		{
			var text = "MacCatalystCompatPlatformDialogRetentionRepro ERROR" + Environment.NewLine + ex;
			File.WriteAllText(ReproSession.ResultsPath, text);
			Console.WriteLine(text);
