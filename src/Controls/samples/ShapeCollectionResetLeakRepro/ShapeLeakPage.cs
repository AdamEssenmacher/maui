...
public sealed class ShapeLeakPage : ContentPage
{
	public ShapeLeakPage()
	{
		...
		var trackedVisuals = new List<TrackedShapeVisual>(options.CardsPerPage);
		...
		{
			var visual = ShapeCardFactory.CreateTrackedPath(options, payload, cycle, i);
		...
		Content = new ScrollView
		{
			Content = new VerticalStackLayout
			{
				...
				Children =
				{
					...
					new Label
					{
						Text = $"{options.Target.DisplayName()}: {options.CardsPerPage} cards, {options.SharedItemsPerCard} transient {options.Target.TransientItemName()} per card, {options.PayloadMegabytesPerPage} MB cached case payload.",
						FontSize = 13,
