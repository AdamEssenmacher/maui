...
internal static class ReproSession
{
	...
	internal const int PayloadKiBPerTextSlot = 256;
	...
	public static readonly string ResultsPath =
		Path.Combine("/tmp", "ios-shell-searchhandler-text-retention-results.txt");

	public static int TotalTextSlots => Cycles * 2;

	public static async Task<ReproReport> RunAsync(IMauiContext context)
	{
		WriteProgress("Starting iOS Shell SearchHandler text retention repro.");
		ForceFullGc();
		var baselineBytes = GC.GetTotalMemory(true);

		WriteProgress("Running control scenario.");
		var control = await RunScenarioAsync(
			"control: clear Shell UISearchBar text slots and remove retained SearchButtonClicked handler",
			context,
			clearNativeText: true,
			removeSearchButtonClickedHandler: true);

		WriteProgress("Running current MAUI scenario.");
		var current = await RunScenarioAsync(
			"current: ShellPageRendererTracker leaves SearchButtonClicked subscribed and text slots assigned",
			context,
			clearNativeText: false,
			removeSearchButtonClickedHandler: false);
	...
	static async Task<ScenarioResult> RunScenarioAsync(
		...
		bool removeSearchButtonClickedHandler)
	{
		...
		for (var i = 0; i < Cycles; i++)
		{
			...
			var cycleResult = await RunCycleAsync(i, context, clearNativeText, removeSearchButtonClickedHandler);
		...
		bool removeSearchButtonClickedHandler)
	{
		...
		{
			Placeholder = CreateLargeText(cycle, "Placeholder"),
			Query = CreateLargeText(cycle, "Query"),
		...
		if (CountAssignedPayloadTextSlots(searchBar) != 2)
			throw new InvalidOperationException("ShellPageRendererTracker did not assign both payload-sized native search text slots.");
		...
		if (removeSearchButtonClickedHandler)
	...
	static string CreateLargeText(int cycle, string slot)
	{
		var header = $"{slot} search text {cycle:0000}. ";
	...
	static int CountAssignedPayloadTextSlots(UISearchBar searchBar)
	{
		var count = 0;

		if (EstimateTextBytes(searchBar.Text) >= PayloadBytesPerTextSlot * 0.95)
			count++;

		if (EstimatePlaceholderBytes(searchBar) >= PayloadBytesPerTextSlot * 0.95)
			count++;

		return count;
	}

	static long EstimateAssignedTextBytes(UISearchBar searchBar)
	{
		return Math.Min(EstimateTextBytes(searchBar.Text), PayloadBytesPerTextSlot) +
			Math.Min(EstimatePlaceholderBytes(searchBar), PayloadBytesPerTextSlot);
	}
		...
		int AlivePageHandlers)
	{
			...
			IReadOnlyList<TrackedCycle> tracked)
		{
			...
			foreach (var retainedSearchBar in retainedSearchBars)
			{
				...
				assignedPayloadTextSlots += CountAssignedPayloadTextSlots(searchBar);
				estimatedAssignedTextBytes += EstimateAssignedTextBytes(searchBar);
			}
	...
	ReproSession.ScenarioResult Current)
{
	public bool LeakProved =>
		...
		Current.AssignedPayloadTextSlots == ReproSession.TotalTextSlots &&
		Current.EstimatedAssignedTextBytes >= ReproSession.TotalTextSlots * PayloadKiBPerTextSlot * 1024L * 0.95 &&
	...
	{
		var currentMiB = Current.EstimatedAssignedTextBytes / 1024d / 1024d;
		var controlMiB = Control.EstimatedAssignedTextBytes / 1024d / 1024d;
		var heapDeltaMiB = (FinalManagedBytes - BaselineManagedBytes) / 1024d / 1024d;

		return string.Join(Environment.NewLine,
			"IosShellSearchHandlerTextRetentionRepro",
			$"Cycles: {Cycles}",
			$"Payload per native text slot: {PayloadKiBPerTextSlot} KiB",
			$"Native text slots per cycle: 2",
			$"Total native text slots: {ReproSession.TotalTextSlots}",
			"Current MAUI detach leaves UISearchBar.SearchButtonClicked subscribed to the tracker.",
			...
			Format(Current),
			string.Empty,
			$"Control estimated retained native search text payload: {controlMiB:N1} MiB",
			$"Current estimated retained native search text payload: {currentMiB:N1} MiB",
	...
	static string Format(ReproSession.ScenarioResult result)
	{
		...
		return string.Join(Environment.NewLine,
			...
			$"  assigned payload-sized text slots: {result.AssignedPayloadTextSlots}/{ReproSession.TotalTextSlots}",
			$"  estimated retained native search text bytes: {result.EstimatedAssignedTextBytes:N0}",
			$"  estimated retained native search text MiB: {nativeTextMiB:N1}",
