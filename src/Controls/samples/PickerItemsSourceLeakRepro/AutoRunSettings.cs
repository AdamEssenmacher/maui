...
internal static class AutoRunSettings
{
	...
	public static void Initialize(string[] args)
	{
		...
		if (string.Equals(Environment.GetEnvironmentVariable("PICKER_ITEMSSOURCE_LEAK_REPRO_AUTORUN"), "1", StringComparison.Ordinal))
			Enabled = true;

		ResultsPath ??= Environment.GetEnvironmentVariable("PICKER_ITEMSSOURCE_LEAK_REPRO_RESULTS");
	}
}
