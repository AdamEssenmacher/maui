...
public sealed class DashboardPage : ContentPage
{
	...
	readonly Entry _pickersEntry;
	readonly Entry _choicesEntry;
	...
	{
		Title = "Picker ItemsSource Leak Repro";
		BackgroundColor = Colors.White;

		_cyclesEntry = CreateEntry("60");
		_pickersEntry = CreateEntry("6");
		_choicesEntry = CreateEntry("80");
		_payloadEntry = CreateEntry("2");
		_dwellEntry = CreateEntry("25");

		_runLeakButton = CreateButton("Run shared source", () => RunAsync(ReproMode.SharedItemsSource));
		_runControlButton = CreateButton("Run control", () => RunAsync(ReproMode.FreshItemsSourceControl));
		_runMitigationButton = CreateButton("Run mitigation", () => RunAsync(ReproMode.ClearSharedItemsSourceOnDisappear));
		...
		_progress = new ProgressBar
		{
			Progress = 0,
			HeightRequest = 6,
			ProgressColor = Color.FromArgb("#245B4F")
		};

		_statusLabel = new Label
		{
			Text = "Ready. Run the control first, then the shared ItemsSource scenario.",
		...
		{
			Text = "Each run pushes real Shell pages with Picker controls, unwinds the stack, forces full GC, and counts which weak references survived.",
			TextColor = Color.FromArgb("#172026"),
		...
		Content = new ScrollView
		{
			Content = new VerticalStackLayout
			{
				...
				Children =
				{
					new Label
					{
						Text = "Shared Picker ItemsSource retention",
						FontSize = 22,
					...
					new Label
					{
						Text = "This models form pages that reuse a shared ObservableCollection for repeated picker choices. A shared ItemsSource should not keep closed pages alive.",
						FontSize = 14,
	...
	Grid CreateSettingsGrid()
	{
		...
		grid.Add(CreateField("Pickers/page", _pickersEntry), 1, 0);
		grid.Add(CreateField("Choices/picker", _choicesEntry), 0, 1);
	...
	static Button CreateButton(string text, Func<Task> action)
	{
		var button = new Button
		{
			Text = text,
			FontSize = 14,
			BackgroundColor = Color.FromArgb("#245B4F"),
	...
	ReproOptions ReadOptions(ReproMode mode)
	{
		return new ReproOptions(
			...
			ReadBoundedInt(_pickersEntry.Text, 1, 64, 6),
			ReadBoundedInt(_choicesEntry.Text, 1, 500, 80),
	...
	async Task RunAsync(ReproMode mode)
	{
		...
		try
		{
			...
			for (var i = 0; i < options.Cycles; i++)
			{
				...
				_statusLabel.Text = $"Pushing picker page {cycle + 1}/{options.Cycles}: {options.Name}";

				await Shell.Current.GoToAsync(AppShell.PickerLeakRoute, animate: false);
			...
			{
				token.ThrowIfCancellationRequested();
				_statusLabel.Text = $"Popping picker page {i + 1}/{options.Cycles}: {options.Name}";
	...
	async Task RunAllScenariosAsync()
	{
		...
		{
			$"PickerItemsSourceLeakRepro autorun started at {DateTimeOffset.Now:O}",
			$"Defaults: pages={_cyclesEntry.Text}, pickersPerPage={_pickersEntry.Text}, choices={_choicesEntry.Text}, payloadMB={_payloadEntry.Text}, dwellMs={_dwellEntry.Text}"
		};

		foreach (var mode in new[]
		{
			ReproMode.FreshItemsSourceControl,
			ReproMode.SharedItemsSource,
			ReproMode.ClearSharedItemsSourceOnDisappear
		})
		{
			await RunAsync(mode);
	...
	static async Task WriteAutoRunReportAsync(string reportText)
	{
		...
		paths.Add(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "PickerItemsSourceLeakRepro", "autorun-results.txt"));
		paths.Add(Path.Combine(Path.GetTempPath(), "pickeritemssourceleakrepro-results.txt"));
