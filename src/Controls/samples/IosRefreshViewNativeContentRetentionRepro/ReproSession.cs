...
using ObjCRuntime;
using UIKit;
using PlatformRefreshView = Microsoft.Maui.Platform.MauiRefreshView;

namespace IosRefreshViewNativeContentRetentionRepro;
...
internal static class ReproSession
{
	...
	static readonly FieldInfo ContentViewField =
		typeof(PlatformRefreshView).GetField("_contentView", BindingFlags.Instance | BindingFlags.NonPublic)
		?? throw new InvalidOperationException("Could not find MauiRefreshView._contentView.");
	static readonly List<IReadOnlyList<RetainedParentPeer>> RetainedNativeParents = new();

	public static readonly string ResultsPath =
		Path.Combine("/tmp", "ios-refreshview-native-content-retention-results.txt");

	public static ReproReport Run(IMauiContext mauiContext)
	{
		WriteProgress("Starting iOS/Mac Catalyst RefreshView native content retention repro.");
		ForceFullGc();
		var baselineBytes = GC.GetTotalMemory(true);

		WriteProgress("Running control scenario.");
		var control = RunScenario(
			"control: retained native MauiRefreshView parents after explicit native content cleanup",
			mauiContext,
			clearNativeContentAfterDisconnect: true);

		WriteProgress("Running current MAUI scenario.");
		var current = RunScenario(
			"current: RefreshViewHandler.DisconnectHandler leaves current native content attached",
			mauiContext,
		...
		bool clearNativeContentAfterDisconnect)
	{
		...
		for (var i = 0; i < Cycles; i++)
		{
			...
			CreateRefreshViewCycle(i, mauiContext, retainedParents, tracked, clearNativeContentAfterDisconnect);
		}
	...
	static void CreateRefreshViewCycle(
		int cycle,
		IMauiContext mauiContext,
		...
		bool clearNativeContentAfterDisconnect)
	{
		...
		var child = new Label
		{
			AutomationId = $"refreshview-child-label-{cycle:000}",
		...
		var parent = new RefreshView
		{
			AutomationId = $"refreshview-parent-{cycle:000}",
			Content = child,
		...
		var handler = (RefreshViewHandler)parent.ToHandler(mauiContext);
		...
		var nativeLabel = FindPayloadLabel(platformParent)
			?? throw new InvalidOperationException("RefreshViewHandler did not attach a native UILabel child with the expected payload.");

		if (GetContentField(platformParent) is null)
			throw new InvalidOperationException("MauiRefreshView._contentView was not assigned after content mapping.");

		var childHandler = child.Handler
			?? throw new InvalidOperationException("RefreshView content Label did not receive a handler.");
	...
	static string CreatePayload(int cycle)
	{
		...
		var sentence =
			$"generated refresh ledger row {cycle:0000} with offline reconciliation notes, approval history, localized state, and audit metadata. ";
	...
	static void ClearNativeContent(PlatformRefreshView platformParent)
	{
		platformParent.RefreshControl.RemoveFromSuperview();
		platformParent.ClearSubviews();
		ContentViewField.SetValue(platformParent, null);
	}

	static UIView? GetContentField(PlatformRefreshView platformParent)
	{
		return ContentViewField.GetValue(platformParent) as UIView;
	}
	...
	static int CountPayloadLabels(UIView view, HashSet<IntPtr> visited, out long estimatedBytes)
	{
		var count = 0;
		estimatedBytes = 0;
		...
		foreach (var subview in view.Subviews)
		{
			count += CountPayloadLabelsIncludingRoot(subview, visited, out var nestedBytes);
	...
	static int CountPayloadLabelsIncludingRoot(UIView view, HashSet<IntPtr> visited, out long estimatedBytes)
	{
		estimatedBytes = 0;
		if (!visited.Add(view.Handle))
			return 0;
		...
		count += CountPayloadLabels(view, visited, out var nestedBytes);
		estimatedBytes += nestedBytes;
		return count;
	}
	...
	static RetainedParentPeer RetainNativeParent(PlatformRefreshView peer)
	{
		var handle = peer.Handle;
		if (handle == IntPtr.Zero)
			throw new InvalidOperationException("Cannot retain a native MauiRefreshView peer with a zero handle.");
	...
	static ParentSnapshot GetParentSnapshot(RetainedParentPeer retainedParent)
	{
		...
		if (parent is null)
		{
			return new ParentSnapshot(
				Alive: false,
				Subviews: 0,
				ContentFieldAssigned: false,
				PayloadLabels: 0,
				EstimatedPayloadBytes: 0);
		}

		var visited = new HashSet<IntPtr>();
		var payloadLabels = CountPayloadLabels(parent, visited, out var estimatedPayloadBytes);
		var contentView = GetContentField(parent);
		if (contentView is not null)
		{
			payloadLabels += CountPayloadLabelsIncludingRoot(contentView, visited, out var contentFieldBytes);
			estimatedPayloadBytes += contentFieldBytes;
		}
	...
	internal sealed record ParentSnapshot(
		bool Alive,
		int Subviews,
		bool ContentFieldAssigned,
		int PayloadLabels,
	...
	internal sealed class RetainedParentPeer
	{
		...
		public PlatformRefreshView? TryGetPeer()
		{
			if (Handle == IntPtr.Zero)
				return null;
			...
			try
			{
				return Runtime.GetNSObject<PlatformRefreshView>(Handle, false);
			}
			catch
			{
				return null;
	...
	ReproSession.ScenarioResult Current)
{
	...
	public string ToText()
	{
		...
		return string.Join(Environment.NewLine,
			"IosRefreshViewNativeContentRetentionRepro",
	...
	static string Format(ReproSession.ScenarioResult result)
	{
		...
		return string.Join(Environment.NewLine,
			...
			$"  retained native MauiRefreshView parents: {result.RetainedNativeParents}/{result.TrackedCycles}",
			$"  retained parents with subviews: {result.ParentsWithSubviews}/{result.TrackedCycles}",
			$"  retained parents with _contentView field: {result.ParentsWithContentField}/{result.TrackedCycles}",
			$"  native UILabel payload views: {result.PayloadNativeLabels}/{result.TrackedCycles}",
			$"  estimated retained native label bytes: {result.EstimatedPayloadBytes:N0}",
			$"  estimated retained native label MiB: {retainedMiB:N1}",
			$"  alive parent RefreshViews: {result.AliveParentViews}/{result.TrackedCycles}",
