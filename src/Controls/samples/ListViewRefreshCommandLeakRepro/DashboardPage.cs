...
public sealed class DashboardPage : ContentPage
{
	...
	{
		Title = "ListView RefreshCommand Leak Repro";
		...
		_runLeakButton = CreateButton("Run shared command", () => RunAsync(ReproMode.SharedRefreshCommand));
		_runControlButton = CreateButton("Run control", () => RunAsync(ReproMode.CreatedPageControl));
		_runMitigationButton = CreateButton("Run cleanup", () => RunAsync(ReproMode.SharedRefreshCommandCleared));
		...
		_progress = new ProgressBar
		{
			Progress = 0,
			HeightRequest = 6,
			ProgressColor = Color.FromArgb("#375A7F")
		};
		...
		{
			Text = "Each run creates ListView pages with payload view models, forces full GC, and counts which weak references survived.",
			TextColor = Color.FromArgb("#172026"),
		...
		Content = new ScrollView
		{
			Content = new VerticalStackLayout
			{
				...
				Children =
				{
					new Label
					{
						Text = "ListView.RefreshCommand strong ICommand retention",
						FontSize = 22,
					...
					new Label
					{
						Text = "This models shared app commands used by pull-to-refresh list pages.",
						FontSize = 14,
	...
	static Button CreateButton(string text, Func<Task> action)
	{
		var button = new Button
		{
			Text = text,
			FontSize = 14,
			BackgroundColor = Color.FromArgb("#375A7F"),
	...
	async Task RunAsync(ReproMode mode)
	{
		...
		try
		{
			...
			for (var i = 0; i < options.Cycles; i++)
			{
				...
				ListViewLeakPage? page = session.CreateTrackedPage();
	...
	async Task RunAllScenariosAsync()
	{
		...
		{
			$"ListViewRefreshCommandLeakRepro autorun started at {DateTimeOffset.Now:O}",
		...
		foreach (var mode in new[]
		{
			...
			ReproMode.SharedRefreshCommand,
			ReproMode.SharedRefreshCommandCleared
		})
		{
			await RunAsync(mode);
	...
	static async Task WriteAutoRunReportAsync(string reportText)
	{
		...
		paths.Add(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "ListViewRefreshCommandLeakRepro", "autorun-results.txt"));
		paths.Add(Path.Combine(Path.GetTempPath(), "listviewrefreshcommandleakrepro-results.txt"));
