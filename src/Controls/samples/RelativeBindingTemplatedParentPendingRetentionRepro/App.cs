...
public class App : Application
{
	...
	const string ResultPath = "/tmp/relativebinding-templatedparent-pending-retention-results.txt";
	...
	protected override Window CreateWindow(IActivationState? activationState) =>
		new(new ContentPage
		{
			Content = new Label
			{
				Text = "Running RelativeBindingSource TemplatedParent pending retention repro...",
	...
	static string RunProof()
	{
		...
		var control = CreateRun("control: parented label completes relative-source templated-parent waits", keepTargetUnparented: false);
		var afterControlCreate = ForceFullGC();
		control.Measure();

		var current = CreateRun("current: retained off-tree label with removed RelativeBindingSource.TemplatedParent bindings", keepTargetUnparented: true);
		...
		builder.AppendLine("RelativeBindingTemplatedParentPendingRetentionRepro");
		builder.AppendLine("Retained target Label instances in both scenarios: 1");
		builder.AppendLine($"RelativeBindingSource.TemplatedParent Binding apply/remove cycles before parenting: {BindingCount}");
		...
		builder.AppendLine();
		builder.AppendLine("Leak path: retained off-tree target Label -> ParentSet event invocation list -> TemplateUtilities.GetRealParentAsync handler -> TaskCompletionSource continuations -> async Binding.ApplyRelativeSourceBinding / RelativeBindingSource.Apply state machine -> removed Binding -> converter -> payload.");
		...
		builder.AppendLine("The current path keeps the target live but off-tree, repeatedly applies and removes RelativeBindingSource.TemplatedParent bindings, and leaves each removed binding captured by an incomplete templated-parent wait.");
	...
	static RunResult CreateRun(string name, bool keepTargetUnparented)
	{
		...
		for (var i = 0; i < BindingCount; i++)
		{
			...
			var binding = new Binding("BindingContext", converter: converter, source: RelativeBindingSource.TemplatedParent);

			target.SetBinding(Label.TextProperty, binding);
			target.RemoveBinding(Label.TextProperty);

			result.BindingRefs.Add(new WeakReference<Binding>(binding));
	...
	static void AppendRun(StringBuilder builder, RunResult result, long beforeBytes, long afterBytes)
	{
		...
		builder.AppendLine($"  removed Bindings alive after full GC: {result.BindingsAlive}/{BindingCount}");
	...
	sealed class RunResult
	{
		...
		public List<WeakReference<Binding>> BindingRefs { get; } = new();
