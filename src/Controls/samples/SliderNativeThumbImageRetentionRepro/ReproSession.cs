...
namespace SliderNativeThumbImageRetentionRepro;
...
internal static class ReproSession
{
	...
	static readonly PropertyMapper<ISlider, ISliderHandler> EmptySliderMapper = new();
	static readonly List<RetainedPeer> RetainedNativePeers = new();

	public static readonly string ResultsPath =
		Path.Combine(Path.GetTempPath(), "slider-native-thumb-image-retention-results.txt");
	...
	public static async Task<ReproReport> RunAsync(IMauiContext context)
	{
		...
		var control = await RunScenarioAsync(
			"control: clear native thumb image before disconnect",
			context,
			clearNativeThumbAndDisposeResult: true);

		var current = await RunScenarioAsync(
			"current: MAUI disconnect leaves native thumb image assigned",
			context,
			clearNativeThumbAndDisposeResult: false);
	...
	static async Task<ScenarioResult> RunScenarioAsync(
		...
		bool clearNativeThumbAndDisposeResult)
	{
		...
		for (var i = 0; i < Cycles; i++)
		{
			...
			var handler = new SliderHandler(EmptySliderMapper);

			AttachHandler(slider, handler, context);

			var platformView = handler.PlatformView;
			var provider = handler.GetRequiredService<IImageSourceServiceProvider>();

			if (clearNativeThumbAndDisposeResult)
			{
				await UpdateThumbImageSourceAndDisposeResultAsync(platformView, slider, provider);
			}
			else
			{
				await platformView.UpdateThumbImageSourceAsync(slider, provider);
			}

			if (platformView.ThumbImage(UIControlState.Normal) is null)
				throw new InvalidOperationException("Slider did not assign a native thumb UIImage.");

			if (clearNativeThumbAndDisposeResult)
				platformView.SetThumbImage(null, UIControlState.Normal);
			...
			tracked.Add(TrackedCycle.Create(i, platformView, slider, handler, source));
		}
		...
		IImageSourceServiceProvider provider)
	{
		...
		if (thumbImageSource is null)
		{
			...
			platformSlider.UpdateThumbColor(slider);
			return;
		}
	...
	internal sealed record RetainedPeer(UISlider Peer);

	internal sealed record TrackedCycle(
		int Cycle,
		WeakReference<UISlider> NativePeer,
		...
		WeakReference<TrackingImageSource> Source)
	{
			...
			TrackingImageSource source)
		{
			return new TrackedCycle(
				cycle,
				new WeakReference<UISlider>(platformView),
		...
		int AliveSources)
	{
			...
			IReadOnlyList<TrackedCycle> tracked)
		{
			...
			{
				if (retainedPeer.Peer.ThumbImage(UIControlState.Normal) is UIImage image)
				{
	...
	ReproSession.ScenarioResult Current)
{
	...
	public string ToText()
	{
		...
		return string.Join(Environment.NewLine,
			"SliderNativeThumbImageRetentionRepro",
			$"Cycles: {Cycles}",
			$"Thumb image size: {SourceImagePixels} x {SourceImagePixels} pixels",
