...
namespace AndroidLegacyTabbedPageRendererTabIconRetentionRepro;
...
internal static class ReproSession
{
	...
	static readonly MethodInfo SetTabIconImageSourceForPageMethod =
		typeof(LegacyTabbedPageRenderer).GetMethod(
			"SetTabIconImageSource",
			BindingFlags.Instance | BindingFlags.NonPublic,
			binder: null,
			types: new[] { typeof(Page), typeof(TabLayout.Tab) },
			modifiers: null)
		?? throw new MissingMethodException(typeof(LegacyTabbedPageRenderer).FullName, "SetTabIconImageSource(Page,TabLayout.Tab)");
		...
		bool clearNativeIconSlots)
	{
		...
		{
			await CreateCycleAsync(activity, TabPlacement.Top, i, retainedPeers, tracked, clearNativeIconSlots);
		...
		bool clearNativeIconSlots)
	{
		...
		var tabLayout = new TabLayout(activity) { TabMode = TabLayout.ModeFixed, TabGravity = TabLayout.GravityFill };
		RendererPlatformField.SetValue(renderer, legacyPlatform);
		TabLayoutField.SetValue(renderer, tabLayout);
		...
		for (var tab = 0; tab < TabsPerPage; tab++)
		{
			...
			var nativeTab = tabLayout.NewTab();
			tabLayout.AddTab(nativeTab, tab == 0);
			SetTabIconImageSourceForPageMethod.Invoke(renderer, new object[] { page, nativeTab });
		}
		...
		var nativePeer = NativePeerRoot.Create(placement, tabLayout);
	...
	ReproSession.ScenarioResult Current)
{
	...
	public int ExpectedBottomPayloadSlots => 0;

	public long ExpectedCurrentRetainedPayloadBytes =>
		(long)ExpectedTopPayloadSlots * PayloadBytesPerIcon;

	public bool LeakProved =>
		Control.AssignedBeforeCleanup == CyclesPerPlacement &&
		Current.AssignedBeforeCleanup == CyclesPerPlacement &&
		Control.AliveTopTabPeers == CyclesPerPlacement &&
		Current.AliveTopTabPeers == CyclesPerPlacement &&
		Control.AliveBottomTabPeers == 0 &&
		Current.AliveBottomTabPeers == 0 &&
		Control.PayloadTopIconSlots == 0 &&
		Control.PayloadBottomIconSlots == 0 &&
		Current.PayloadTopIconSlots >= ExpectedTopPayloadSlots &&
		Current.PayloadBottomIconSlots == 0 &&
	...
	public string ToText()
	{
		...
		return string.Join(Environment.NewLine,
			"AndroidLegacyTabbedPageRendererTabIconRetentionRepro",
			...
			"Source path exercised: obsolete Android TabbedPageRenderer.SetTabIconImageSource(Page, TabLayout.Tab) -> TabLayout.Tab.SetIcon",
			"Native roots retained after disposal: JNI global refs to TabLayout peers only",
	...
	static string Format(ReproSession.ScenarioResult result)
	{
		return string.Join(Environment.NewLine,
			...
			$"  retained native TabLayout peers: {result.AliveTopTabPeers}/{result.TrackedCycles}",
			$"  retained native BottomNavigationView peers: {result.AliveBottomTabPeers}/0",
