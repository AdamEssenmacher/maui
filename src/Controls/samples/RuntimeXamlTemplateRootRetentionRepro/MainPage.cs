...
public sealed class MainPage : ContentPage
{
	...
	const string RuntimeXaml = """
		<ContentPage xmlns="http://schemas.microsoft.com/dotnet/2021/maui"
		  ...
		  <ContentPage.Resources>
		    <DataTemplate x:Key="EscapedTemplate">
		      <Grid Padding="4">
		        <Label Text="Runtime XAML template content" />
		      </Grid>
		    </DataTemplate>
		  </ContentPage.Resources>
		</ContentPage>
	...
	public MainPage(string? resultsPath)
	{
		...
		Title = "Runtime XAML Template Root Retention";

		_status = new Label
		{
			Text = "Running runtime XAML template root-retention repro...",
	...
	async void OnLoaded(object? sender, EventArgs e)
	{
		...
		try
		{
			...
			_status.Text = result.Proven
				? "PROVEN: escaped runtime-XAML DataTemplates retained discarded page roots."
		...
		{
			var report = "Runtime XAML DataTemplate root-retention repro failed." + Environment.NewLine + ex;
			_status.Text = report;
		...
		List<WeakReference<byte[]>> payloadBufferRefs)
	{
		...
		if (page.Resources["EscapedTemplate"] is not DataTemplate template)
			throw new InvalidOperationException("The runtime XAML page did not contain the expected DataTemplate resource.");
	...
	readonly record struct ReproResult(ScenarioResult Control, ScenarioResult Current, bool Proven)
	{
		public string ToReport()
		{
			...
			builder.AppendLine("Runtime XAML DataTemplate root-retention repro");
			...
			builder.AppendLine("Escaped DataTemplate -> runtime-XAML LoadTemplate closure -> outer HydrationContext -> RootElement ContentPage -> BindingContext payload");
