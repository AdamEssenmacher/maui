...
sealed class RunnerPage : ContentPage
{
	...
	public RunnerPage()
	{
		Content = new Label
		{
			Text = "Running iOS compatibility image renderer native image retention repro...",
	...
	async Task TryRunAsync()
	{
		...
		{
			var text = "IosCompatImageRendererNativeImageRetentionRepro ERROR" + Environment.NewLine + ex;
			File.WriteAllText(ReproSession.ResultsPath, text);
			Console.WriteLine(text);
		}
