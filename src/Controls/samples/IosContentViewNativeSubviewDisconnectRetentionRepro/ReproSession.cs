...
internal static class ReproSession
{
	...
	public static readonly string ResultsPath =
		Path.Combine("/tmp", "ios-contentview-native-subview-disconnect-retention-results.txt");

	public static ReproReport Run(IMauiContext mauiContext)
	{
		WriteProgress("Starting iOS/Mac Catalyst ContentView native subview disconnect retention repro.");
		ForceFullGc();
		...
		var current = RunScenario(
			"current: ContentViewHandler.DisconnectHandler leaves current native child subview attached",
			mauiContext,
		...
		bool clearSubviewsAfterDisconnect)
	{
		...
		for (var i = 0; i < Cycles; i++)
		{
			...
			CreateContentViewCycle(i, mauiContext, retainedParents, tracked, clearSubviewsAfterDisconnect);
		}
	...
	static void CreateContentViewCycle(
		int cycle,
		IMauiContext mauiContext,
		...
		bool clearSubviewsAfterDisconnect)
	{
		...
		var child = new Label
		{
			AutomationId = $"contentview-child-label-{cycle:000}",
		...
		var parent = new MauiContentView
		{
			AutomationId = $"contentview-parent-{cycle:000}",
			Content = child,
			WidthRequest = 900,
			HeightRequest = 40
		};
		parent.Layout(new Rect(0, 0, 900, 40));
		child.Layout(new Rect(0, 0, 900, 40));

		var handler = (ContentViewHandler)parent.ToHandler(mauiContext);
		var platformParent = handler.PlatformView;
		SetRealisticBounds(platformParent, 900, 40);
		handler.UpdateValue(nameof(IContentView.Content));

		var nativeLabel = FindPayloadLabel(platformParent)
			?? throw new InvalidOperationException("ContentView did not attach a native UILabel child with the expected payload.");

		var childHandler = child.Handler
			?? throw new InvalidOperationException("ContentView child Label did not receive a handler.");
	...
	static string CreatePayload(int cycle)
	{
		...
		var sentence =
			$"generated order audit row {cycle:0000} with offline comments, shipping notes, localized status, and triage metadata. ";
	...
	static RetainedParentPeer RetainNativeParent(PlatformContentView peer)
	{
		var handle = peer.Handle;
		if (handle == IntPtr.Zero)
			throw new InvalidOperationException("Cannot retain a native ContentView peer with a zero handle.");
	...
	ReproSession.ScenarioResult Current)
{
	...
	public string ToText()
	{
		...
		return string.Join(Environment.NewLine,
			"IosContentViewNativeSubviewDisconnectRetentionRepro",
	...
	static string Format(ReproSession.ScenarioResult result)
	{
		...
		return string.Join(Environment.NewLine,
			...
			$"  alive parent ContentViews: {result.AliveParentViews}/{result.TrackedCycles}",
