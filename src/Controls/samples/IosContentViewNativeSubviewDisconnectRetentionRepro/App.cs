...
sealed class RunnerPage : ContentPage
{
	...
	public RunnerPage()
	{
		Content = new Label
		{
			Text = "Running iOS/Mac Catalyst ContentView native subview disconnect retention repro...",
	...
	async Task TryRunAsync()
	{
		...
		{
			var text = "IosContentViewNativeSubviewDisconnectRetentionRepro ERROR" + Environment.NewLine + ex;
			File.WriteAllText(ReproSession.ResultsPath, text);
			Console.WriteLine(text);
			await Task.Delay(250);
			Environment.Exit(3);
