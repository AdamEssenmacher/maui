...
public sealed class DashboardPage : ContentPage
{
	...
	readonly Entry _brushesEntry;
	readonly Entry _stopsEntry;
	...
	{
		Title = "GradientBrush GradientStops Leak Repro";
		BackgroundColor = Colors.White;

		_cyclesEntry = CreateEntry("60");
		_brushesEntry = CreateEntry("6");
		_stopsEntry = CreateEntry("12");
		_payloadEntry = CreateEntry("2");
		_dwellEntry = CreateEntry("25");

		_runLeakButton = CreateButton("Run shared stops", () => RunAsync(ReproMode.SharedGradientStops));
		_runControlButton = CreateButton("Run control", () => RunAsync(ReproMode.FreshGradientStopsControl));
		_runMitigationButton = CreateButton("Run mitigation", () => RunAsync(ReproMode.ClearSharedGradientStopsOnDisappear));
		...
		_progress = new ProgressBar
		{
			Progress = 0,
			HeightRequest = 6,
			ProgressColor = Color.FromArgb("#3D6B61")
		};

		_statusLabel = new Label
		{
			Text = "Ready. Run the control first, then the shared GradientStopCollection scenario.",
		...
		{
			Text = "Each run pushes real Shell pages with gradient-backed surfaces, unwinds the stack, forces full GC, and counts which weak references survived.",
			TextColor = Color.FromArgb("#172026"),
		...
		Content = new ScrollView
		{
			Content = new VerticalStackLayout
			{
				...
				Children =
				{
					new Label
					{
						Text = "Shared GradientStopCollection retention",
						FontSize = 22,
					...
					new Label
					{
						Text = "This models dashboards that create per-page gradient brushes from a shared design-system gradient stop collection.",
						FontSize = 14,
	...
	Grid CreateSettingsGrid()
	{
		...
		grid.Add(CreateField("Brushes/page", _brushesEntry), 1, 0);
		grid.Add(CreateField("Stops/brush", _stopsEntry), 0, 1);
	...
	static Button CreateButton(string text, Func<Task> action)
	{
		var button = new Button
		{
			Text = text,
			FontSize = 14,
			BackgroundColor = Color.FromArgb("#3D6B61"),
	...
	ReproOptions ReadOptions(ReproMode mode)
	{
		return new ReproOptions(
			...
			ReadBoundedInt(_brushesEntry.Text, 1, 64, 6),
			ReadBoundedInt(_stopsEntry.Text, 2, 128, 12),
	...
	async Task RunAsync(ReproMode mode)
	{
		...
		try
		{
			...
			for (var i = 0; i < options.Cycles; i++)
			{
				...
				_statusLabel.Text = $"Pushing gradient page {cycle + 1}/{options.Cycles}: {options.Name}";
			...
			{
				token.ThrowIfCancellationRequested();
				_statusLabel.Text = $"Popping gradient page {i + 1}/{options.Cycles}: {options.Name}";
	...
	async Task RunAllScenariosAsync()
	{
		...
		{
			$"GradientBrushGradientStopsLeakRepro autorun started at {DateTimeOffset.Now:O}",
			$"Defaults: pages={_cyclesEntry.Text}, brushesPerPage={_brushesEntry.Text}, stops={_stopsEntry.Text}, payloadMB={_payloadEntry.Text}, dwellMs={_dwellEntry.Text}"
		};

		foreach (var mode in new[]
		{
			ReproMode.FreshGradientStopsControl,
			ReproMode.SharedGradientStops,
			ReproMode.ClearSharedGradientStopsOnDisappear
		})
		{
			await RunAsync(mode);
	...
	static async Task WriteAutoRunReportAsync(string reportText)
	{
		...
		paths.Add(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "GradientBrushGradientStopsLeakRepro", "autorun-results.txt"));
		paths.Add(Path.Combine(Path.GetTempPath(), "gradientbrushgradientstopsleakrepro-results.txt"));
