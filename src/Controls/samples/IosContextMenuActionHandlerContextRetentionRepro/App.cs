...
sealed class RunnerPage : ContentPage
{
	...
	public RunnerPage()
	{
		Content = new Label
		{
			Text = "Running iOS context menu UIAction handler context retention repro...",
	...
	async Task TryRunAsync()
	{
		...
		{
			var text = "IosContextMenuActionHandlerContextRetentionRepro ERROR" + Environment.NewLine + ex;
			File.WriteAllText(ReproSession.ResultsPath, text);
			Console.WriteLine(text);
