...
public sealed class DashboardPage : ContentPage
{
	...
	{
		Title = "DisplayRotationStateTrigger Leak Repro";
		...
		_runLeakButton = CreateButton("Run live VSM replacement", () => RunAsync(ReproMode.LiveThemeSwap));
		_runControlButton = CreateButton("Run static VSM control", () => RunAsync(ReproMode.StaticThemeControl));
		_runPreloadedThemeButton = CreateButton("Run preloaded VSM replacement", () => RunAsync(ReproMode.PreloadedThemeSwap));
		...
		{
			Text = "Ready. Run the static rotation-theme control first, then the live VSM swap scenario.",
		...
		{
			Text = "Each run pushes real Shell pages, simulates a live visual-state refresh on rotation-aware dashboard cards, unwinds the stack, forces full GC, and counts which weak references survived.",
			TextColor = Color.FromArgb("#172026"),
		...
		Content = new ScrollView
		{
			Content = new VerticalStackLayout
			{
				...
				Children =
				{
					new Label
					{
						Text = "DisplayRotationStateTrigger VSM replacement retention",
						FontSize = 22,
					...
					new Label
					{
						Text = "This models tenant or brand theme refresh on visible rotation-aware controls. Static XAML visual states should clean up normally.",
						FontSize = 14,
	...
	async Task RunAsync(ReproMode mode)
	{
		...
		try
		{
			...
			for (var i = 0; i < options.Cycles; i++)
			{
				...
				_statusLabel.Text = $"Pushing rotation-aware page {cycle + 1}/{options.Cycles}: {options.Name}";

				await Shell.Current.GoToAsync(AppShell.DisplayRotationStateTriggerLeakRoute, animate: false);
			...
			{
				token.ThrowIfCancellationRequested();
				_statusLabel.Text = $"Popping rotation-aware page {i + 1}/{options.Cycles}: {options.Name}";
	...
	async Task RunAllScenariosAsync()
	{
		...
		{
			$"DisplayRotationStateTriggerLeakRepro autorun started at {DateTimeOffset.Now:O}",
	...
	static async Task WriteAutoRunReportAsync(string reportText)
	{
		...
		paths.Add(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "DisplayRotationStateTriggerLeakRepro", "autorun-results.txt"));
		paths.Add(Path.Combine(Path.GetTempPath(), "displayrotationstatetriggerleakrepro-results.txt"));
