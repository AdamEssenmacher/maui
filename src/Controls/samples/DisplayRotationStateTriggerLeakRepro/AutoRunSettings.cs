...
internal static class AutoRunSettings
{
	...
	public static void Initialize(string[] args)
	{
		...
		if (string.Equals(Environment.GetEnvironmentVariable("DISPLAY_ROTATION_STATE_TRIGGER_LEAK_REPRO_AUTORUN"), "1", StringComparison.Ordinal))
			Enabled = true;

		ResultsPath ??= Environment.GetEnvironmentVariable("DISPLAY_ROTATION_STATE_TRIGGER_LEAK_REPRO_RESULTS");
	}
}
