...
sealed class RunnerPage : ContentPage
{
	...
	public RunnerPage()
	{
		Content = new Label
		{
			Text = "Running iOS TabbedRenderer child page subscription retention repro...",
	...
	async Task TryRunAsync()
	{
		...
		{
			var text = "IosTabbedRendererChildPageSubscriptionRetentionRepro ERROR" + Environment.NewLine + ex;
			File.WriteAllText(ReproSession.ResultsPath, text);
			Console.WriteLine(text);
		}
