...
public sealed class OrientationStateTriggerLeakPage : ContentPage
{
	readonly ContentView _target;
	bool _readySignaled;

	public OrientationStateTriggerLeakPage()
	{
		...
		var initialTheme = CreateOrientationGroups("Contoso orientation theme", Color.FromArgb("#E8F6F8"));
		VisualStateManager.SetVisualStateGroups(_target, initialTheme.Groups);

		if (options.PreloadSwappedTheme)
			VisualStateManager.SetVisualStateGroups(_target, CreateOrientationGroups("Northwind preloaded theme", Color.FromArgb("#F0F5EB")).Groups);
	...
	async Task CompletePageSetupAsync()
	{
		...
		if (session.Options.SwapThemeAfterAttach)
			VisualStateManager.SetVisualStateGroups(_target, CreateOrientationGroups("Northwind live theme", Color.FromArgb("#F8F0E8")).Groups);
	...
	static View CreateTargetContent(LeakPayloadViewModel payload)
	{
		...
		return new VerticalStackLayout
		{
			...
			Children =
			{
				...
				new Label
				{
					Text = "Orientation-aware dashboard content with cached report payload. The old state trigger carries this view model through BindingContext.",
					FontSize = 14,
					TextColor = Color.FromArgb("#57606A")
				},
	...
	static OrientationTheme CreateOrientationGroups(string label, Color activeColor)
	{
		var groups = new VisualStateGroupList();
		var group = new VisualStateGroup { Name = "OrientationStates" };
		...
		groups.Add(group);
		return new OrientationTheme(groups, trigger);
	}

	readonly record struct OrientationTheme(VisualStateGroupList Groups, OrientationStateTrigger Trigger);
