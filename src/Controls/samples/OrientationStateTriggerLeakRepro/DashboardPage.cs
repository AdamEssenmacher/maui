...
public sealed class DashboardPage : ContentPage
{
	...
	{
		Title = "OrientationStateTrigger Leak Repro";
		...
		{
			Text = "Ready. Run the static orientation-theme control first, then the live VSM swap scenario.",
		...
		{
			Text = "Each run pushes real Shell pages, simulates a live visual-state refresh on orientation-aware dashboard cards, unwinds the stack, forces full GC, and counts which weak references survived.",
			TextColor = Color.FromArgb("#172026"),
		...
		Content = new ScrollView
		{
			Content = new VerticalStackLayout
			{
				...
				Children =
				{
					new Label
					{
						Text = "OrientationStateTrigger VSM replacement retention",
						FontSize = 22,
					...
					new Label
					{
						Text = "This models tenant or brand theme refresh on visible orientation-aware controls. Static XAML visual states should clean up normally.",
						FontSize = 14,
	...
	async Task RunAsync(ReproMode mode)
	{
		...
		try
		{
			...
			for (var i = 0; i < options.Cycles; i++)
			{
				...
				_statusLabel.Text = $"Pushing orientation-aware page {cycle + 1}/{options.Cycles}: {options.Name}";

				await Shell.Current.GoToAsync(AppShell.OrientationStateTriggerLeakRoute, animate: false);
			...
			{
				token.ThrowIfCancellationRequested();
				_statusLabel.Text = $"Popping orientation-aware page {i + 1}/{options.Cycles}: {options.Name}";
	...
	async Task RunAllScenariosAsync()
	{
		...
		{
			$"OrientationStateTriggerLeakRepro autorun started at {DateTimeOffset.Now:O}",
	...
	static async Task WriteAutoRunReportAsync(string reportText)
	{
		...
		paths.Add(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "OrientationStateTriggerLeakRepro", "autorun-results.txt"));
		paths.Add(Path.Combine(Path.GetTempPath(), "orientationstatetriggerleakrepro-results.txt"));
