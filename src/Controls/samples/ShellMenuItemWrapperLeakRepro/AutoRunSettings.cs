...
internal static class AutoRunSettings
{
	...
	{
		return ResultsPath ?? Path.Combine(Path.GetTempPath(), "shellmenuitemwrapperleakrepro-results.txt");
	}
	...
	public static void Initialize(string[] args)
	{
		...
		if (string.Equals(Environment.GetEnvironmentVariable("SHELL_MENU_ITEM_WRAPPER_LEAK_REPRO_AUTORUN"), "1", StringComparison.Ordinal))
			Enabled = true;

		ResultsPath ??= Environment.GetEnvironmentVariable("SHELL_MENU_ITEM_WRAPPER_LEAK_REPRO_RESULTS");
	}
}
