...
public sealed class MainPage : ContentPage
{
	...
	public MainPage()
	{
		_status = new Label
		{
			Text = "Running ShellContent.MenuItems.Clear retention leak repro...",
			Margin = 24
		};
	...
	static async Task WriteResultsAsync(string text)
	{
		...
		resultsPath ??= Path.Combine(Path.GetTempPath(), "shellcontentmenuitemsclearretentionleakrepro-results.txt");
