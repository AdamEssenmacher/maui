...
internal static class AutoRunSettings
{
	...
	{
		return ResultsPath ?? Path.Combine(Path.GetTempPath(), "borderchildhandlerdisconnectleakrepro-results.txt");
	}
	...
	public static void Initialize(string[] args)
	{
		...
		if (string.Equals(Environment.GetEnvironmentVariable("BORDER_CHILD_HANDLER_DISCONNECT_LEAK_REPRO_AUTORUN"), "1", StringComparison.Ordinal))
			Enabled = true;

		ResultsPath ??= Environment.GetEnvironmentVariable("BORDER_CHILD_HANDLER_DISCONNECT_LEAK_REPRO_RESULTS");
	}
}
