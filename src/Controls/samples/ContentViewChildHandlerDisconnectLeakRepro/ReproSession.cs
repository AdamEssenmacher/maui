...
internal static class ReproSession
{
	...
	static ScenarioResult RunCurrentContentReplacement(IMauiContext mauiContext)
	{
		...
		var result = ScenarioResult.From("current ContentViewHandler: retained removed content after Content = null", tracked);
		GC.KeepAlive(retainedRemovedContent);
		return result;
	}
		...
		int cycle)
	{
		...
		host.Content = null;
		hostHandler.UpdateValue(nameof(IContentView.Content));
	...
	static Microsoft.Maui.Controls.ContentView CreateHost(PayloadContentView content)
	{
	...
	static IElementHandler AttachHost(IMauiContext mauiContext, Microsoft.Maui.Controls.ContentView host)
	{
	...
	public LeakPayload(int cycle, long payloadBytes)
	{
		...
		CachedCards = Enumerable.Range(1, 24)
			.Select(index => new CachedCard(
				...
				$"Offline ContentView child payload {index}",
