...
sealed class RunnerPage : ContentPage
{
	...
	public RunnerPage()
	{
		Content = new Label
		{
			Text = "Running PhoneFlyoutPageRenderer background pattern retention repro...",
	...
	async Task TryRunAsync()
	{
		...
		{
			var text = "PhoneFlyoutPageBackgroundPatternRetentionRepro ERROR" + Environment.NewLine + ex;
			File.WriteAllText(ReproSession.ResultsPath, text);
			Console.WriteLine(text);
		}
