...
sealed class RunnerPage : ContentPage
{
	...
	public RunnerPage()
	{
		Content = new Label
		{
			Text = "Running iOS CollectionView supplementary label retention repro...",
	...
	async Task TryRunAsync()
	{
		...
		{
			var text = "IosCollectionViewSupplementaryLabelRetentionRepro ERROR" + Environment.NewLine + ex;
			File.WriteAllText(ReproSession.ResultsPath, text);
			Console.WriteLine(text);
		}
