...
using ObjCRuntime;
using UIKit;
using CompatibilityEditorRenderer = Microsoft.Maui.Controls.Compatibility.Platform.iOS.EditorRenderer;
using CompatibilityEntryRenderer = Microsoft.Maui.Controls.Compatibility.Platform.iOS.EntryRenderer;

namespace IosCompatTextRendererRetentionRepro;
...
internal static class ReproSession
{
	...
	public static readonly string ResultsPath =
		Path.Combine("/tmp", "ios-compat-text-renderer-retention-results.txt");

	public static ReproReport Run(IMauiContext mauiContext)
	{
		WriteProgress("Starting iOS compatibility text renderer retention repro.");
		ForceFullGc();
		...
		bool clearNativeTextBeforeDispose)
	{
		...
		for (var i = 0; i < CyclesPerRendererType; i++)
		{
			...
			CreateEntryRendererCycle(i, mauiContext, retainedPeers, tracked, clearNativeTextBeforeDispose);
			CreateEditorRendererCycle(i, mauiContext, retainedPeers, tracked, clearNativeTextBeforeDispose);
		}
	...
	static void CreateEntryRendererCycle(
		int cycle,
		IMauiContext mauiContext,
		...
		bool clearNativeTextBeforeDispose)
	{
		...
		var entry = new Entry
		{
			AutomationId = $"compat-entry-renderer-text-{cycle:000}",
			Text = text,
			Placeholder = "Customer note lookup and pasted claim detail",
			CharacterSpacing = 0.1,
			WidthRequest = 720,
			HeightRequest = 48
		};
		entry.Layout(new Rect(0, 0, 720, 48));

		var contextHandler = new ContextOnlyHandler(mauiContext);
		contextHandler.SetVirtualView(entry);

		var renderer = new CompatibilityEntryRenderer();
		SetRealisticBounds(renderer, 720, 48);
		renderer.SetElement(entry);

		var nativeTextField = renderer.Control
			?? throw new InvalidOperationException("EntryRenderer did not create a native UITextField.");
		nativeTextField.Frame = new CGRect(0, 0, 720, 48);

		if (!NativeTextHasPayload(nativeTextField))
			throw new InvalidOperationException("EntryRenderer did not assign the expected native text payload.");

		var retainedPeer = RetainNativePeer("Entry", nativeTextField);
		...
		tracked.Add(TrackedCycle.Create("Entry", cycle, renderer, entry));
	}

	[MethodImpl(MethodImplOptions.NoInlining)]
	static void CreateEditorRendererCycle(
		int cycle,
		IMauiContext mauiContext,
		List<RetainedTextPeer> retainedPeers,
		List<TrackedCycle> tracked,
		bool clearNativeTextBeforeDispose)
	{
		...
		{
			AutomationId = $"compat-editor-renderer-text-{cycle:000}",
			Text = text,
		...
		editor.Layout(new Rect(0, 0, 720, 240));

		var contextHandler = new ContextOnlyHandler(mauiContext);
		contextHandler.SetVirtualView(editor);

		var renderer = new CompatibilityEditorRenderer();
		SetRealisticBounds(renderer, 720, 240);
		renderer.SetElement(editor);

		var nativeTextView = renderer.Control
			?? throw new InvalidOperationException("EditorRenderer did not create a native UITextView.");
		nativeTextView.Frame = new CGRect(0, 0, 720, 240);

		if (!NativeTextHasPayload(nativeTextView))
			throw new InvalidOperationException("EditorRenderer did not assign the expected native text payload.");

		var retainedPeer = RetainNativePeer("Editor", nativeTextView);

		if (clearNativeTextBeforeDispose)
			ClearNativeText(nativeTextView);
		...
		retainedPeers.Add(retainedPeer);
		tracked.Add(TrackedCycle.Create("Editor", cycle, renderer, editor));
	}

	static string CreateDocumentText(string rendererType, int cycle)
	{
		var header = $"Cycle {cycle:000} {rendererType} retained customer document text. ";
		var sentence = "This offline workflow stores copied claim notes, generated summaries, and searchable audit text. ";
	...
	static bool NativeTextHasPayload(UITextView textView) =>
		EstimateTextBytes(textView.Text, textView.AttributedText?.Value) >= PayloadBytesPerControl * 0.95;
	...
	static void ClearNativeText(UITextView textView)
	{
		textView.Text = null;
		textView.AttributedText = new NSAttributedString(string.Empty);
	...
	static NativeTextSnapshot GetNativeTextSnapshot(RetainedTextPeer retainedPeer)
	{
		...
		return peer switch
		{
			...
			UITextView textView => new NativeTextSnapshot(
				Alive: true,
				EstimateTextBytes(textView.Text, textView.AttributedText?.Value) +
					EstimateSubviewTextBytes(textView)),
			_ => new NativeTextSnapshot(Alive: false, EstimatedTextBytes: 0)
		};
	}

	static long EstimateSubviewTextBytes(UITextView textView)
	{
		long bytes = 0;
		foreach (var subview in textView.Subviews)
		{
			if (subview is UILabel label)
				bytes += EstimateTextBytes(label.Text, label.AttributedText?.Value);
		}
		...
		public NSObject? TryGetPeer()
		{
			...
			try
			{
				return ControlType == "Entry"
					? Runtime.GetNSObject<UITextField>(Handle, false)
					: Runtime.GetNSObject<UITextView>(Handle, false);
			}
			catch
			{
				return null;
	...
	ReproSession.ScenarioResult Current)
{
	...
	public bool LeakProved =>
		...
		Current.ByControlType.TryGetValue("Editor", out var editor) &&
		editor.NativePeersWithText == CyclesPerRendererType &&
		Current.ByControlType.TryGetValue("Entry", out var entry) &&
		entry.NativePeersWithText == CyclesPerRendererType;
	...
	public string ToText()
	{
		...
		return string.Join(Environment.NewLine,
			"IosCompatTextRendererRetentionRepro",
