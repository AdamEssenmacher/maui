...
sealed class RunnerPage : ContentPage
{
	...
	public RunnerPage()
	{
		Content = new Label
		{
			Text = "Running iOS compatibility renderer MauiContext retention repro...",
	...
	async Task TryRunAsync()
	{
		...
		{
			var text = "IosCompatibilityRendererMauiContextRetentionRepro ERROR" + Environment.NewLine + ex;
			File.WriteAllText(ReproSession.ResultsPath, text);
			Console.WriteLine(text);
		}
