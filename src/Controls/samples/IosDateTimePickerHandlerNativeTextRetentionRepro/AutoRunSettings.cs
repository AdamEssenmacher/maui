...
internal static class AutoRunSettings
{
	...
	{
		return ResultsPath ?? Path.Combine(Path.GetTempPath(), "ios-datetimepickerhandler-native-text-retention-results.txt");
	}
	...
	public static void Initialize(string[] args)
	{
		...
		if (string.Equals(Environment.GetEnvironmentVariable("IOS_DATETIMEPICKER_NATIVE_TEXT_RETENTION_REPRO_AUTORUN"), "1", StringComparison.Ordinal))
			Enabled = true;

		ResultsPath ??= Environment.GetEnvironmentVariable("IOS_DATETIMEPICKER_NATIVE_TEXT_RETENTION_REPRO_RESULTS");
	}
}
