...
internal static class ReproSession
{
	...
	public static ReproReport Run(IMauiContext mauiContext)
	{
		...
		var leak = RunCurrentSwipeItemContentReplacement(mauiContext);
	...
	static ScenarioResult RunCurrentSwipeItemContentReplacement(IMauiContext mauiContext)
	{
		var retainedRemovedContent = new List<PayloadSwipeContentView>();
		var tracked = new List<TrackedCycle>();
		...
		var result = ScenarioResult.From("current SwipeItemViewHandler: retained removed swipe content after Content = null", tracked);
		GC.KeepAlive(retainedRemovedContent);
		return result;
	}
		...
		int cycle)
	{
		...
		host.Content = null;
		hostHandler.UpdateValue(nameof(ISwipeItemView.Content));
	...
	static Microsoft.Maui.Controls.SwipeItemView CreateHost(PayloadSwipeContentView content)
	{
		return new Microsoft.Maui.Controls.SwipeItemView
		{
			WidthRequest = 360,
			HeightRequest = 80,
			Content = content
	...
	static IElementHandler AttachHost(IMauiContext mauiContext, Microsoft.Maui.Controls.SwipeItemView host)
	{
...
internal sealed class LeakPayload
{
	public LeakPayload(int cycle, long payloadBytes)
	{
		...
		CachedSwipeActions = Enumerable.Range(1, 24)
			.Select(index => new CachedSwipeAction(
				$"ACTION-{cycle + 1:000}-{index:000}",
				$"Offline swipe action payload {index}",
	...
	public IReadOnlyList<CachedSwipeAction> CachedSwipeActions { get; }
}

internal sealed record CachedSwipeAction(string Id, string Summary, string Status);
	...
	ScenarioResult Leak)
{
	...
	public string ToText()
	{
		return string.Join(Environment.NewLine,
			"SwipeItemView child handler disconnect leak repro",
			$"Cycles: {Cycles}",
