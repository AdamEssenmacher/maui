...
sealed class RunnerPage : ContentPage
{
	...
	public RunnerPage()
	{
		Content = new Label
		{
			Text = "Running iOS compatibility DatePickerRenderer/TimePickerRenderer native text retention repro...",
	...
	async Task TryRunAsync()
	{
		...
		{
			var text = "IosCompatDateTimePickerRendererNativeTextRetentionRepro ERROR" + Environment.NewLine + ex;
			File.WriteAllText(ReproSession.ResultsPath, text);
			Console.WriteLine(text);
			await Task.Delay(250);
			Environment.Exit(3);
