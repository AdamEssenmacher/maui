...
internal static class ReproSession
{
	...
	public static ReproReport Run(IMauiContext mauiContext)
	{
		...
		var leak = RunCurrentRefreshContentReplacement(mauiContext);
	...
	{
		var retainedRemovedContent = new List<PayloadRefreshContentView>();
		...
		var result = ScenarioResult.From("control: retained removed refresh content after explicit child DisconnectHandler", tracked);
		GC.KeepAlive(retainedRemovedContent);
		return result;
	}

	static ScenarioResult RunCurrentRefreshContentReplacement(IMauiContext mauiContext)
	{
		var retainedRemovedContent = new List<PayloadRefreshContentView>();
		var tracked = new List<TrackedCycle>();
		...
		var result = ScenarioResult.From("current RefreshViewHandler: retained removed refresh content after Content = null", tracked);
		GC.KeepAlive(retainedRemovedContent);
		return result;
	}
	...
	static void CreateControlCycle(
		...
		List<PayloadRefreshContentView> retainedRemovedContent,
		List<TrackedCycle> tracked,
		int cycle)
	{
		var removedContent = new PayloadRefreshContentView(cycle);
		retainedRemovedContent.Add(removedContent);

		var host = CreateHost(removedContent);
		var hostHandler = AttachHost(mauiContext, host);
		var handler = (PayloadRefreshContentViewHandler)removedContent.Handler!;
		...
		host.Content = null;
		hostHandler.UpdateValue(nameof(IRefreshView.Content));
	...
	static Microsoft.Maui.Controls.RefreshView CreateHost(PayloadRefreshContentView content)
	{
		return new Microsoft.Maui.Controls.RefreshView
		{
			WidthRequest = 360,
			HeightRequest = 80,
			Content = content
	...
	static IElementHandler AttachHost(IMauiContext mauiContext, Microsoft.Maui.Controls.RefreshView host)
	{
...
internal sealed class PayloadRefreshContentView : View
{
	public PayloadRefreshContentView(int cycle)
	{
		Cycle = cycle;
		HeightRequest = 80;
		WidthRequest = 360;
...
internal sealed class PayloadRefreshContentViewHandler : ViewHandler<PayloadRefreshContentView, PayloadNativeView>
{
	public static readonly IPropertyMapper<PayloadRefreshContentView, PayloadRefreshContentViewHandler> Mapper =
		new PropertyMapper<PayloadRefreshContentView, PayloadRefreshContentViewHandler>(ViewHandler.ViewMapper);

	public PayloadRefreshContentViewHandler() : base(Mapper)
	{
	}
...
internal sealed class LeakPayload
{
	public LeakPayload(int cycle, long payloadBytes)
	{
		...
		CachedRefreshCards = Enumerable.Range(1, 24)
			.Select(index => new CachedRefreshCard(
				$"REFRESH-{cycle + 1:000}-{index:000}",
				$"Offline refresh content payload {index}",
	...
	public IReadOnlyList<CachedRefreshCard> CachedRefreshCards { get; }
}

internal sealed record CachedRefreshCard(string Id, string Summary, string Status);
	...
	long PayloadBytes)
{
	public static TrackedCycle Create(
		int cycle,
		PayloadRefreshContentView retainedView,
		PayloadRefreshContentViewHandler handler,
	...
	ScenarioResult Leak)
{
	...
	static string FormatScenario(ScenarioResult result)
	{
		...
		return string.Join(Environment.NewLine,
			...
			$"  Retained removed refresh content views alive: {result.AliveRetainedVirtualViews}/{result.TrackedCycles}",
