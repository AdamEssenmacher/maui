...
internal static class AutoRunSettings
{
	...
	public static void Initialize(string[] args)
	{
		...
		if (string.Equals(Environment.GetEnvironmentVariable("RESOURCE_DICTIONARY_MERGED_DICTIONARIES_LEAK_REPRO_AUTORUN"), "1", StringComparison.Ordinal))
			Enabled = true;

		ResultsPath ??= Environment.GetEnvironmentVariable("RESOURCE_DICTIONARY_MERGED_DICTIONARIES_LEAK_REPRO_RESULTS");
	}
}
