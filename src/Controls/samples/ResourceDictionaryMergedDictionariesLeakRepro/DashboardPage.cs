...
public sealed class DashboardPage : ContentPage
{
	...
	readonly Entry _sharedResourcesEntry;
	readonly Entry _localResourcesEntry;
	...
	{
		Title = "ResourceDictionary MergedDictionaries Leak Repro";
		BackgroundColor = Colors.White;

		_cyclesEntry = CreateEntry("60");
		_sharedResourcesEntry = CreateEntry("80");
		_localResourcesEntry = CreateEntry("20");
		_payloadEntry = CreateEntry("2");
		_dwellEntry = CreateEntry("25");

		_runLeakButton = CreateButton("Run shared dictionary", () => RunAsync(ReproMode.SharedMergedDictionary));
		_runControlButton = CreateButton("Run control", () => RunAsync(ReproMode.FreshMergedDictionaryControl));
		_runMitigationButton = CreateButton("Run mitigation", () => RunAsync(ReproMode.ClearMergedDictionariesOnDisappear));
		...
		_progress = new ProgressBar
		{
			Progress = 0,
			HeightRequest = 6,
			ProgressColor = Color.FromArgb("#4F5C7A")
		};

		_statusLabel = new Label
		{
			Text = "Ready. Run the control first, then the shared merged dictionary scenario.",
		...
		{
			Text = "Each run pushes real Shell pages with page ResourceDictionaries, unwinds the stack, forces full GC, and counts which weak references survived.",
			TextColor = Color.FromArgb("#172026"),
		...
		Content = new ScrollView
		{
			Content = new VerticalStackLayout
			{
				...
				Children =
				{
					new Label
					{
						Text = "Shared merged ResourceDictionary retention",
						FontSize = 22,
					...
					new Label
					{
						Text = "This models repeated pages that merge the same app-level design-system ResourceDictionary into page-local resources.",
						FontSize = 14,
	...
	Grid CreateSettingsGrid()
	{
		...
		grid.Add(CreateField("Shared resources", _sharedResourcesEntry), 1, 0);
		grid.Add(CreateField("Page resources", _localResourcesEntry), 0, 1);
	...
	static Button CreateButton(string text, Func<Task> action)
	{
		var button = new Button
		{
			Text = text,
			FontSize = 14,
			BackgroundColor = Color.FromArgb("#4F5C7A"),
	...
	ReproOptions ReadOptions(ReproMode mode)
	{
		return new ReproOptions(
			...
			ReadBoundedInt(_sharedResourcesEntry.Text, 1, 500, 80),
			ReadBoundedInt(_localResourcesEntry.Text, 0, 500, 20),
	...
	async Task RunAsync(ReproMode mode)
	{
		...
		try
		{
			...
			for (var i = 0; i < options.Cycles; i++)
			{
				...
				_statusLabel.Text = $"Pushing resource page {cycle + 1}/{options.Cycles}: {options.Name}";
			...
			{
				token.ThrowIfCancellationRequested();
				_statusLabel.Text = $"Popping resource page {i + 1}/{options.Cycles}: {options.Name}";
	...
	async Task RunAllScenariosAsync()
	{
		...
		{
			$"ResourceDictionaryMergedDictionariesLeakRepro autorun started at {DateTimeOffset.Now:O}",
			$"Defaults: pages={_cyclesEntry.Text}, sharedResources={_sharedResourcesEntry.Text}, localResources={_localResourcesEntry.Text}, payloadMB={_payloadEntry.Text}, dwellMs={_dwellEntry.Text}"
		};

		foreach (var mode in new[]
		{
			ReproMode.FreshMergedDictionaryControl,
			ReproMode.SharedMergedDictionary,
			ReproMode.ClearMergedDictionariesOnDisappear
		})
		{
			await RunAsync(mode);
	...
	static async Task WriteAutoRunReportAsync(string reportText)
	{
		...
		paths.Add(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "ResourceDictionaryMergedDictionariesLeakRepro", "autorun-results.txt"));
		paths.Add(Path.Combine(Path.GetTempPath(), "resourcedictionarymergeddictionariesleakrepro-results.txt"));
