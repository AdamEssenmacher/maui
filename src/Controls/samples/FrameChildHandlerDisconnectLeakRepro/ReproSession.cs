...
internal static class ReproSession
{
	...
	public static ReproReport Run(IMauiContext mauiContext)
	{
		...
		var leak = RunCurrentContentReplacement(mauiContext);
	...
	{
		var retainedRemovedContent = new List<PayloadFrameContentView>();
		...
		var result = ScenarioResult.From("control: retained removed content after explicit child DisconnectHandler", tracked);
		GC.KeepAlive(retainedRemovedContent);
		return result;
	}

	static ScenarioResult RunCurrentContentReplacement(IMauiContext mauiContext)
	{
		var retainedRemovedContent = new List<PayloadFrameContentView>();
		var tracked = new List<TrackedCycle>();
		...
		var result = ScenarioResult.From("current FrameRenderer: retained removed content after Content = null", tracked);
		GC.KeepAlive(retainedRemovedContent);
		return result;
	}
	...
	static void CreateControlCycle(
		...
		List<PayloadFrameContentView> retainedRemovedContent,
		List<TrackedCycle> tracked,
		int cycle)
	{
		var removedContent = new PayloadFrameContentView(cycle);
		retainedRemovedContent.Add(removedContent);

		var host = CreateHost(removedContent);
		var hostHandler = AttachHost(mauiContext, host);
		var handler = (PayloadFrameContentViewHandler)removedContent.Handler!;
		...
		host.Content = null;
		hostHandler.UpdateValue(Microsoft.Maui.Controls.Frame.ContentProperty.PropertyName);
	...
	static Microsoft.Maui.Controls.Frame CreateHost(PayloadFrameContentView content)
	{
		return new Microsoft.Maui.Controls.Frame
		{
			WidthRequest = 360,
			HeightRequest = 80,
			Content = content
	...
	static IElementHandler AttachHost(IMauiContext mauiContext, Microsoft.Maui.Controls.Frame host)
	{
		var handler = host.ToHandler(mauiContext);
		if (handler.PlatformView is UIView view)
			view.Frame = new CGRect(0, 0, 360, 80);

		handler.UpdateValue(Microsoft.Maui.Controls.Frame.ContentProperty.PropertyName);
		return handler;
	}
...
internal sealed class PayloadFrameContentView : View
{
	public PayloadFrameContentView(int cycle)
	{
		Cycle = cycle;
		HeightRequest = 80;
		WidthRequest = 360;
...
internal sealed class PayloadFrameContentViewHandler : ViewHandler<PayloadFrameContentView, PayloadNativeView>
{
	public static readonly IPropertyMapper<PayloadFrameContentView, PayloadFrameContentViewHandler> Mapper =
		new PropertyMapper<PayloadFrameContentView, PayloadFrameContentViewHandler>(ViewHandler.ViewMapper);

	public PayloadFrameContentViewHandler() : base(Mapper)
	{
	}
	...
	public LeakPayload(int cycle, long payloadBytes)
	{
		...
		CachedCards = Enumerable.Range(1, 24)
			.Select(index => new CachedCard(
				...
				$"Offline Frame child payload {index}",
	...
	long PayloadBytes)
{
	public static TrackedCycle Create(
		int cycle,
		PayloadFrameContentView retainedView,
		PayloadFrameContentViewHandler handler,
	...
	ScenarioResult Leak)
{
	...
	public string ToText()
	{
		return string.Join(Environment.NewLine,
			"Frame child handler disconnect leak repro",
			$"Cycles: {Cycles}",
	...
	static string FormatScenario(ScenarioResult result)
	{
		...
		return string.Join(Environment.NewLine,
			...
			$"  Retained removed content views alive: {result.AliveRetainedVirtualViews}/{result.TrackedCycles}",
