...
sealed class RunnerPage : ContentPage
{
	...
	public RunnerPage()
	{
		Content = new Label
		{
			Text = "Running iOS VisualElementRenderer MauiContext retention repro...",
	...
	async Task TryRunAsync()
	{
		...
		{
			var text = "IosVisualElementRendererMauiContextRetentionRepro ERROR" + Environment.NewLine + ex;
			File.WriteAllText(ReproSession.ResultsPath, text);
			Console.WriteLine(text);
		}
