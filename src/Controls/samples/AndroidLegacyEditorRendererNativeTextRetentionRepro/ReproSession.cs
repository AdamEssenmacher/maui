...
namespace AndroidLegacyEditorRendererNativeTextRetentionRepro;
...
internal static class ReproSession
{
	...
	public static async Task<ReproReport> RunAsync(IMauiContext context, Element contextRoot)
	{
		...
		var control = await RunScenarioAsync(
			"control: dispose EditorRenderer after clearing native EditText.Text and EditText.Hint",
			context,
			contextRoot,
			clearNativeText: true);

		var current = await RunScenarioAsync(
			"current: dispose EditorRenderer without clearing native text slots",
			context,
			contextRoot,
		...
		bool clearNativeText)
	{
		...
		{
			Text = CreatePayload("editor-text", cycle, TextPayloadCharsPerSlot),
			Placeholder = CreatePayload("editor-placeholder", cycle, HintPayloadCharsPerSlot),
		...
		var renderer = new EditorRenderer(context.Context ?? throw new InvalidOperationException("Android context is not available."));

		contextRoot.AddLogicalChild(editor);
		try
		{
			((IVisualElementRenderer)renderer).SetElement(editor);
		}
		finally
		{
			contextRoot.RemoveLogicalChild(editor);
		}

		var nativeEditText = renderer.Control
			?? throw new InvalidOperationException("EditorRenderer did not create a native EditText.");
	...
	{
		var textPrefix = $"android-legacy-editorrenderer-{prefix}-{cycle:D4}-";
	...
	internal sealed record TrackedCycle(
		...
		WeakReference<EditorRenderer> ManagedRenderer,
		...
		int AssignedHintLengthBeforeCleanup)
	{
			...
			int assignedHintLengthBeforeCleanup)
		{
			return new TrackedCycle(
				cycle,
				nativeEditText,
				new WeakReference<EditorRenderer>(renderer),
				new WeakReference<Editor>(editor),
				assignedTextLengthBeforeCleanup,
				assignedHintLengthBeforeCleanup);
		}
	}
		...
		long RetainedNativeTextBytes)
	{
		internal static ScenarioResult From(string name, IReadOnlyList<TrackedCycle> tracked)
		{
			...
			foreach (var cycle in tracked)
			{
				...
				if (cycle.Editor.TryGetTarget(out _))
					aliveEditors++;
			}
	...
	ReproSession.ScenarioResult Current)
{
	public bool LeakProved =>
		...
		Control.AliveEditors == 0 &&
		Current.AliveEditors == 0 &&
	...
	public string ToText()
	{
		...
		return string.Join(Environment.NewLine,
			"AndroidLegacyEditorRendererNativeTextRetentionRepro",
			$"Cycles: {Cycles}",
			...
			"Source path exercised: obsolete Android EditorRenderer.UpdateText/UpdatePlaceholderText -> child EditText.Text/EditText.Hint",
	...
	static string Format(ReproSession.ScenarioResult result)
	{
		return string.Join(Environment.NewLine,
			...
			$"  alive Editors after full GC: {result.AliveEditors}/{result.TrackedCycles}",
