...
public sealed class MainPage : ContentPage
{
	...
	public MainPage()
	{
		_status = new Label
		{
			Text = "Running Android legacy EditorRenderer native text retention repro...",
			Margin = 24
		};
	...
	async void OnLoaded(object? sender, EventArgs e)
	{
		...
		try
		{
			...
			var result = await ReproSession.RunAsync(new MauiContext(context.Services, androidContext), this);
			text = result.ToText();
		}
		catch (Exception ex)
		{
