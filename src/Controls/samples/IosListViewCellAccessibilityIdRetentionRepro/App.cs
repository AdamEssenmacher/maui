...
sealed class RunnerPage : ContentPage
{
	...
	public RunnerPage()
	{
		Content = new Label
		{
			Text = "Running iOS ListView cell accessibility identifier retention repro...",
	...
	async Task TryRunAsync()
	{
		...
		{
			var text = "IosListViewCellAccessibilityIdRetentionRepro ERROR" + Environment.NewLine + ex;
			File.WriteAllText(ReproSession.ResultsPath, text);
			Console.WriteLine(text);
		}
