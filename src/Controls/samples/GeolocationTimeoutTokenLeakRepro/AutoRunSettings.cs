...
internal static class AutoRunSettings
{
	...
	{
		return ResultsPath ?? Path.Combine(Path.GetTempPath(), "geolocationtimeouttokenleakrepro-results.txt");
	}
	...
	public static void Initialize(string[] args)
	{
		...
		if (string.Equals(Environment.GetEnvironmentVariable("GEOLOCATION_TIMEOUT_TOKEN_LEAK_REPRO_AUTORUN"), "1", StringComparison.Ordinal))
			Enabled = true;

		ResultsPath ??= Environment.GetEnvironmentVariable("GEOLOCATION_TIMEOUT_TOKEN_LEAK_REPRO_RESULTS");
	}
}
