...
public sealed class DashboardPage : ContentPage
{
	...
	{
		Title = "VisualElement.Resources Leak Repro";
		BackgroundColor = Colors.White;

		_cyclesEntry = CreateEntry("60");
		_sharedResourcesEntry = CreateEntry("120");
		_payloadEntry = CreateEntry("2");
		_dwellEntry = CreateEntry("25");

		_runLeakButton = CreateButton("Run shared Resources", () => RunAsync(ReproMode.SharedResourcesDictionary));
		_runControlButton = CreateButton("Run control", () => RunAsync(ReproMode.FreshResourcesDictionaryControl));
		_runMitigationButton = CreateButton("Run mitigation", () => RunAsync(ReproMode.ReplaceResourcesOnDisappear));
		...
		_progress = new ProgressBar
		{
			Progress = 0,
			HeightRequest = 6,
			ProgressColor = Color.FromArgb("#2F6F73")
		};
		...
		{
			Text = "Each run pushes real Shell pages, assigns page Resources, unwinds the stack, forces full GC, and counts which weak references survived.",
			TextColor = Color.FromArgb("#172026"),
		...
		Content = new ScrollView
		{
			Content = new VerticalStackLayout
			{
				...
				Children =
				{
					new Label
					{
						Text = "Shared VisualElement.Resources retention",
						FontSize = 22,
					...
					new Label
					{
						Text = "This models pages that directly reuse a long-lived app or feature ResourceDictionary as their Resources dictionary.",
						FontSize = 14,
	...
	static Button CreateButton(string text, Func<Task> action)
	{
		var button = new Button
		{
			Text = text,
			FontSize = 14,
			BackgroundColor = Color.FromArgb("#2F6F73"),
	...
	ReproOptions ReadOptions(ReproMode mode)
	{
		return new ReproOptions(
			...
			ReadBoundedInt(_sharedResourcesEntry.Text, 1, 500, 120),
	...
	async Task RunAllScenariosAsync()
	{
		...
		{
			$"VisualElementResourcesLeakRepro autorun started at {DateTimeOffset.Now:O}",
			$"Defaults: pages={_cyclesEntry.Text}, sharedResources={_sharedResourcesEntry.Text}, payloadMB={_payloadEntry.Text}, dwellMs={_dwellEntry.Text}"
		};

		foreach (var mode in new[]
		{
			ReproMode.FreshResourcesDictionaryControl,
			ReproMode.SharedResourcesDictionary,
			ReproMode.ReplaceResourcesOnDisappear
		})
		{
			await RunAsync(mode);
	...
	static async Task WriteAutoRunReportAsync(string reportText)
	{
		...
		paths.Add(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "VisualElementResourcesLeakRepro", "autorun-results.txt"));
		paths.Add(Path.Combine(Path.GetTempPath(), "visualelementresourcesleakrepro-results.txt"));
