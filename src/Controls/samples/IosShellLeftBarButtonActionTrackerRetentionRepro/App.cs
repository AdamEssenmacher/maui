...
sealed class RunnerPage : ContentPage
{
	...
	public RunnerPage()
	{
		Content = new Label
		{
			Text = "Running iOS Shell left bar button action tracker retention repro...",
	...
	async Task TryRunAsync()
	{
		...
		{
			var text = "IosShellLeftBarButtonActionTrackerRetentionRepro ERROR" + Environment.NewLine + ex;
			File.WriteAllText(ReproSession.ResultsPath, text);
			Console.WriteLine(text);
		}
