...
internal static class AutoRunSettings
{
	...
	public static void Initialize(string[] args)
	{
		...
		if (string.Equals(Environment.GetEnvironmentVariable("MAP_ELEMENTS_SYNC_PERF_REPRO_AUTORUN"), "1", StringComparison.Ordinal))
			Enabled = true;

		ResultsPath ??= Environment.GetEnvironmentVariable("MAP_ELEMENTS_SYNC_PERF_REPRO_RESULTS");
	}
	...
	public static void AppendTextBlock(string title, string text)
	{
		var path = GetResultsPath();
		var directory = Path.GetDirectoryName(path);
