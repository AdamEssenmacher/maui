...
sealed class RunnerPage : ContentPage
{
	...
	public RunnerPage()
	{
		Content = new Label
		{
			Text = "Running iOS VisualElementRenderer accessibility retention repro...",
	...
	async Task TryRunAsync()
	{
		...
		{
			var text = "IosVisualElementRendererAccessibilityRetentionRepro ERROR" + Environment.NewLine + ex;
			File.WriteAllText(ReproSession.ResultsPath, text);
			Console.WriteLine(text);
