...
internal static class ReproSession
{
	...
	static readonly List<IReadOnlyList<TabbedRenderer>> RetainedRenderers = new();

	public static readonly string ResultsPath =
		Path.Combine("/tmp", "ios-tabbedrenderer-mauicontext-retention-results.txt");

	public static async Task<ReproReport> RunAsync(IMauiContext context)
	{
		WriteProgress("Starting iOS TabbedRenderer MauiContext retention repro.");
		ForceFullGc();
		var baselineBytes = GC.GetTotalMemory(true);

		WriteProgress("Running control scenario.");
		var control = await RunScenarioAsync(
			"control: clear retained TabbedRenderer._mauiContext",
			context,
			clearRendererContext: true);

		WriteProgress("Running current MAUI scenario.");
		var current = await RunScenarioAsync(
			"current: TabbedRenderer keeps _mauiContext after disconnect/dispose",
			context,
		...
		bool clearRendererContext)
	{
		...
		tracked.Add(TrackedCycle.Create(cycle, renderer, tabbedPage, cycleContext, payloadProvider, payload, payload.Buffer));
...
sealed class PayloadTabbedPage : TabbedPage
{
	public PayloadTabbedPage(int cycle)
	{
		...
		AutomationId = $"tabbed-renderer-context-{cycle + 1}";
	...
	ReproSession.ScenarioResult Current)
{
	...
	public string ToText()
	{
		...
		return string.Join(Environment.NewLine,
			"IosTabbedRendererMauiContextRetentionRepro",
			$"Cycles: {Cycles}",
