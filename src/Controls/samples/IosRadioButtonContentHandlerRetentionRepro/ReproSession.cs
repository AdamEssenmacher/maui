...
internal static class ReproSession
{
	...
	{
		var retainedRemovedContent = new List<PayloadContentView>();
		...
		var result = ScenarioResult.From("control: retained removed RadioButton content after explicit child DisconnectHandler", tracked);
	...
	static ScenarioResult RunCurrentContentReplacement(IMauiContext mauiContext)
	{
		...
		var result = ScenarioResult.From("current RadioButtonHandler: retained removed content after Content = null", tracked);
		GC.KeepAlive(retainedRemovedContent);
		return result;
	}
	...
	static void CreateControlCycle(
		...
		List<PayloadContentView> retainedRemovedContent,
		List<TrackedCycle> tracked,
		int cycle)
	{
		var removedContent = new PayloadContentView(cycle);
		retainedRemovedContent.Add(removedContent);

		var host = CreateHost(removedContent);
		var hostHandler = AttachHost(mauiContext, host);
		var handler = (PayloadContentViewHandler)removedContent.Handler!;
		...
		host.Content = null;
		hostHandler.UpdateValue(nameof(IRadioButton.Content));
	...
	static RadioButton CreateHost(PayloadContentView content)
	{
	...
	static IElementHandler AttachHost(IMauiContext mauiContext, RadioButton host)
	{
...
internal sealed class PayloadContentView : View
{
	public PayloadContentView(int cycle)
	{
		Cycle = cycle;
		HeightRequest = 80;
		WidthRequest = 360;
...
internal sealed class PayloadContentViewHandler : ViewHandler<PayloadContentView, PayloadNativeView>
{
	public static readonly IPropertyMapper<PayloadContentView, PayloadContentViewHandler> Mapper =
		new PropertyMapper<PayloadContentView, PayloadContentViewHandler>(ViewHandler.ViewMapper);

	public PayloadContentViewHandler() : base(Mapper)
	{
	}
	...
	public LeakPayload(int cycle, long payloadBytes)
	{
		...
		CachedCards = Enumerable.Range(1, 24)
			.Select(index => new CachedCard(
				...
				$"Offline RadioButton content payload {index}",
	...
	long PayloadBytes)
{
	public static TrackedCycle Create(
		int cycle,
		PayloadContentView retainedView,
		PayloadContentViewHandler handler,
	...
	ScenarioResult Leak)
{
	...
	public string ToText()
	{
		return string.Join(Environment.NewLine,
			...
			$"Cycles: {Cycles}",
			$"Payload per removed RadioButton content handler: {PayloadMegabytesPerCycle} MiB",
