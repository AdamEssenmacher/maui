...
public sealed class DashboardPage : ContentPage
{
	...
	{
		Title = "TransformGroup Leak Repro";
		BackgroundColor = Colors.White;

		_cyclesEntry = CreateEntry("40");
		_pathsEntry = CreateEntry("12");
		_payloadEntry = CreateEntry("3");
		_dwellEntry = CreateEntry("25");

		_runLeakButton = CreateButton("Run shared transform", () => RunAsync(ReproMode.SharedTransform));
		_runControlButton = CreateButton("Run control", () => RunAsync(ReproMode.PrivateTransformControl));
		_runMitigationButton = CreateButton("Run mitigation", () => RunAsync(ReproMode.RemoveSharedTransformBeforeReplace));
		...
		_progress = new ProgressBar
		{
			Progress = 0,
			HeightRequest = 6,
			ProgressColor = Color.FromArgb("#203A43")
		};

		_statusLabel = new Label
		{
			Text = "Ready. Run the control first, then the shared transform scenario.",
		...
		{
			Text = "Each run pushes real Shell pages containing vector Paths, unwinds the stack, forces full GC, and counts which weak references survived.",
			TextColor = Color.FromArgb("#172026"),
		...
		Content = new ScrollView
		{
			Content = new VerticalStackLayout
			{
				...
				Children =
				{
					new Label
					{
						Text = "TransformGroup.Children retention",
						FontSize = 22,
					...
					new Label
					{
						Text = "This uses vector-heavy dashboard pages with cached payloads. Replacing a TransformGroup.Children collection should not leave shared child transforms retaining closed views.",
						FontSize = 14,
	...
	Grid CreateSettingsGrid()
	{
		...
		grid.Add(CreateField("Paths/page", _pathsEntry), 1, 0);
		grid.Add(CreateField("Payload MB/page", _payloadEntry), 0, 1);
	...
	static Button CreateButton(string text, Func<Task> action)
	{
		var button = new Button
		{
			Text = text,
			FontSize = 14,
			BackgroundColor = Color.FromArgb("#203A43"),
	...
	ReproOptions ReadOptions(ReproMode mode)
	{
		return new ReproOptions(
			...
			ReadBoundedInt(_pathsEntry.Text, 1, 48, 12),
	...
	async Task RunAsync(ReproMode mode)
	{
		...
		try
		{
			...
			for (var i = 0; i < options.Cycles; i++)
			{
				...
				_statusLabel.Text = $"Pushing vector page {cycle + 1}/{options.Cycles}: {options.Name}";

				await Shell.Current.GoToAsync(AppShell.TransformLeakRoute, animate: false);
			...
			{
				token.ThrowIfCancellationRequested();
				_statusLabel.Text = $"Popping vector page {i + 1}/{options.Cycles}: {options.Name}";
	...
	async Task RunAllScenariosAsync()
	{
		...
		{
			$"TransformGroupLeakRepro autorun started at {DateTimeOffset.Now:O}",
			$"Defaults: pages={_cyclesEntry.Text}, pathsPerPage={_pathsEntry.Text}, payloadMB={_payloadEntry.Text}, dwellMs={_dwellEntry.Text}"
		};

		foreach (var mode in new[]
		{
			ReproMode.PrivateTransformControl,
			ReproMode.SharedTransform,
			ReproMode.RemoveSharedTransformBeforeReplace
		})
		{
			await RunAsync(mode);
	...
	static async Task WriteAutoRunReportAsync(string reportText)
	{
		...
		paths.Add(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "TransformGroupLeakRepro", "autorun-results.txt"));
		paths.Add(Path.Combine(Path.GetTempPath(), "transformgroupleakrepro-results.txt"));
		...
		foreach (var path in paths.Distinct())
		{
			try
			{
				...
				Console.WriteLine($"Wrote autorun report to {path}");
				return;
			}
			catch (Exception ex)
			{
