...
internal static class ReproSession
{
	...
	static readonly FieldInfo IsHeaderOrFooterChangedField =
		typeof(TemplatedCell2).GetField("isHeaderOrFooterChanged", BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.Public)
		?? throw new MissingFieldException(typeof(TemplatedCell2).FullName, "isHeaderOrFooterChanged");

	static readonly FieldInfo IsSupplementaryViewField =
		typeof(TemplatedCell2).GetField("isSupplementaryView", BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.Public)
		?? throw new MissingFieldException(typeof(TemplatedCell2).FullName, "isSupplementaryView");
	...
	public static ReproReport Run(IMauiContext mauiContext)
	{
		...
		var leak = RunTemplatedCell2ReplacementGap(mauiContext);
	...
	static ScenarioResult RunExplicitDisconnectControl(IMauiContext mauiContext)
	{
		...
		var retainedOldViews = new List<PayloadSupplementaryView>();
		...
		var result = ScenarioResult.From("explicit old supplementary handler disconnect before replacement", tracked);
		GC.KeepAlive(retainedItemsViews);
		GC.KeepAlive(retainedOldViews);
		return result;
	}

	static ScenarioResult RunTemplatedCell2ReplacementGap(IMauiContext mauiContext)
	{
		var retainedItemsViews = new List<CollectionView>();
		var retainedOldViews = new List<PayloadSupplementaryView>();
		var tracked = new List<TrackedCycle>();
		...
		var result = ScenarioResult.From("TemplatedCell2 supplementary replacement without old-handler disconnect", tracked);
		GC.KeepAlive(retainedItemsViews);
		GC.KeepAlive(retainedOldViews);
		return result;
	}
	...
	static void CreateControlCycle(
		...
		List<PayloadSupplementaryView> retainedOldViews,
		...
		int cycle)
	{
		...
		var cell = new TemplatedCell2(new CGRect(0, 0, 360, 56));
		var oldHeader = new PayloadSupplementaryView(cycle, "old header");
		var newHeader = new PayloadSupplementaryView(cycle, "new header");
		retainedOldViews.Add(oldHeader);

		BindAsSupplementary(cell, oldHeader, itemsView);
		var oldHandler = (PayloadSupplementaryViewHandler)oldHeader.Handler!;

		tracked.Add(TrackedCycle.Create(cycle, oldHeader, oldHandler, (PayloadNativeView)oldHandler.PlatformView!, oldHandler.Payload));

		((IElementHandler)oldHandler).DisconnectHandler();
		BindAsSupplementary(cell, newHeader, itemsView);
	}
	...
	static void BindAsSupplementary(TemplatedCell2 cell, View view, CollectionView itemsView)
	{
		IsHeaderOrFooterChangedField.SetValue(cell, true);
		IsSupplementaryViewField.SetValue(cell, true);

		try
		{
			cell.Bind(view, itemsView);
		}
		finally
		{
			IsHeaderOrFooterChangedField.SetValue(cell, false);
		}
	}
...
internal sealed class PayloadSupplementaryView : View
{
	public PayloadSupplementaryView(int cycle, string role)
	{
		Cycle = cycle;
		Role = role;
		HeightRequest = 56;
...
internal sealed class PayloadSupplementaryViewHandler : ViewHandler<PayloadSupplementaryView, PayloadNativeView>
{
	public static readonly IPropertyMapper<PayloadSupplementaryView, PayloadSupplementaryViewHandler> Mapper =
		new PropertyMapper<PayloadSupplementaryView, PayloadSupplementaryViewHandler>(ViewHandler.ViewMapper);

	public PayloadSupplementaryViewHandler() : base(Mapper)
	{
	}
	...
	public LeakPayload(int cycle, long payloadBytes)
	{
		...
		CachedRows = Enumerable.Range(1, 40)
			.Select(index => new CachedRow(
				...
				$"Offline CollectionView header payload {index}",
				"Ready for reuse"))
			.ToArray();
	}
	...
	long PayloadBytes)
{
	public static TrackedCycle Create(
		int cycle,
		PayloadSupplementaryView retainedView,
		PayloadSupplementaryViewHandler handler,
	...
	ScenarioResult Leak)
{
	...
	public string ToText()
	{
		return string.Join(Environment.NewLine,
			...
			$"Cycles: {Cycles}",
			$"Payload per removed supplementary handler: {PayloadMegabytesPerCycle} MiB",
	...
	static string FormatScenario(ScenarioResult result)
	{
		...
		return string.Join(Environment.NewLine,
			...
			$"  Tracked removed views: {result.TrackedCycles}",
			$"  Retained removed views alive: {result.AliveRetainedOldViews}/{result.TrackedCycles}",
			$"  Removed view handlers alive: {result.AliveHandlers}/{result.TrackedCycles}",
