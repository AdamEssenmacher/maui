...
public sealed class DashboardPage : ContentPage
{
	...
	{
		Title = "Path.Data Shared Geometry Leak Repro";
	...
	protected override void OnAppearing()
	{
		...
		if (AutoRunSettings.ShouldRunPathDataSharedGeometry && !_autoRunStarted)
		{
	...
	async Task RunAsync(ReproMode mode)
	{
		...
		try
		{
			...
			for (var i = 0; i < options.Cycles; i++)
			{
				...
				await Shell.Current.GoToAsync(AppShell.PathDataLeakRoute, animate: false);
	...
	async Task RunAllScenariosAsync()
	{
		...
		{
			$"PathDataSharedGeometryLeakRepro autorun started at {DateTimeOffset.Now:O}",
			$"Target: {AutoRunSettings.TargetName}",
			$"Defaults: pages={_cyclesEntry.Text}, payloadMB={_payloadEntry.Text}, pathsPerPage={_pathsEntry.Text}, dwellMs={_dwellEntry.Text}",
			"Leak path: Application.Resources shared PathGeometry / ScaleTransform -> Path.Data / Path.RenderTransform subscriptions -> Path -> dashboard page / BindingContext payload"
		};

		foreach (var mode in new[]
		{
			ReproMode.FreshSourcesControl,
			ReproMode.SharedSources,
			ReproMode.ClearPathSourcesOnDisappear
		})
		{
			await RunAsync(mode);
	...
	static async Task WriteAutoRunReportAsync(string reportText)
	{
		...
		if (!string.IsNullOrWhiteSpace(documentsPath))
			paths.Add(System.IO.Path.GetFullPath(System.IO.Path.Combine(documentsPath, "..", "Library", "PathDataSharedGeometryLeakRepro", "autorun-results.txt")));
#endif

		paths.Add(System.IO.Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "PathDataSharedGeometryLeakRepro", "autorun-results.txt"));
		paths.Add(System.IO.Path.Combine(System.IO.Path.GetTempPath(), "pathdatasharedgeometryleakrepro-results.txt"));

		Exception? lastException = null;
		var wroteReport = false;
		...
		if (!wroteReport && lastException is not null)
